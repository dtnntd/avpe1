...
def export_windows(
    ...
    appFilename = buildApplication(
        appName + ".py", version, source_directory=source_directory, verbose=verbose
    )

    iconfiles = set(glob.glob(icon))
    iconFilename = os.path.abspath(icon)
    zipFilename = zip_file.format(
        name=appName, version="_{:02d}_{:02d}".format(*version)
    )
    ...
    backupFiles(
        (
            ...
            distribution,
            appName,
            os.path.join(specPath, appName) + ".spec",
            zipFilename,
        ),
    ...
    if zipFilename:
        ...
        if verbose > 0:
            print(
                "Put {}application in {} archive".format(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-s", "--source", default=".", help="Directory of source files and PNG images"
    )
    parser.add_argument(
        "-i",
        "--icon",
    ...
    parser.add_argument(
        ...
        "--sign-certificate",
        default="",
        help="Signer certificate file or subject name of cert in certificate " "store",
    )
    parser.add_argument(
        "-p",
        "--password",
