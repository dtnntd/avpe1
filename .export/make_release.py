...
repository_directory = os.path.dirname(os.path.abspath(os.path.dirname(sys.argv[0])))
...
def make_release(
    ...
    if kind is not None and kind not in version_labels:
        raise ValueError(
            "Kind of release, {}, is not in {}".format(kind, version_labels)
        )
    if kind:
        labelIndex = version_labels.index(kind)
        newVersion = [
            version[i] if i < labelIndex else version[i] + 1 if i == labelIndex else 0
    ...
    if repo.is_dirty() and not force:
        ...
        print(
            "Ignoring unsaved changes to git working directory: {}".format(
                repo.working_dir
            )
        )
    ...
    if newVersion != version:
        ...
        if matchingTags:
            if force:
                if verbose > 1:
                    print(
                        "Deleting {} existing tag{} named {!r}".format(
                            len(matchingTags),
                            "" if len(matchingTags) == 1 else "s",
                            tagName,
                        )
                    )
        ...
        if verbose > 0:
            print("Wrote new version, {}, to {}".format(newVersionString, version_path))
        ...
        print(
            "Commit of new {} file resulted in:".format(version_path),
            commit,
            "which is now tagged {!r}".format(newTag.name),
        )

    if os.path.exists(releaseDirectory):
        if not os.path.isdir(releaseDirectory):
            raise Exception(
                "Release directory is not a directory: {}".format(releaseDirectory)
            )
        if backup:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=["all"],
        help=(
            "Export release to target platform. Possible platforms: {}. "
            "Some platforms can only be exported when run on that platform"
    ...
    make_release(
        ...
        force=args.force,
        version_filename=args.version_file,
        prefix=args.prefix,
        targets=args.target,
        repoDir=args.repository_directory,
        backup=args.backup,
