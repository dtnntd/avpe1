...
def commandLineArg(thing):
    ...
    return (
        "{!r}".format(thing) if isinstance(thing, str) and " " in thing else str(thing)
    )
...
def getVersion(version_file, verbose=0):
    ...
    if verbose > 0:
        print(
            (
                "Version read from {}".format(version_file)
                if os.path.exists(version_file)
                else "Version"
            ),
            "is {}.{}".format(*version),
...
def buildApplication(
    ...
    exportPyFiles = set(
        ...
        not in ("runAllVisualizations.py", "runAllVisualizationsMenu.py")
    )
    ...
    if any(len(s) == 0 for s in (visualizations, pngfiles)):
        ...
        for files, kind in ((visualizations, "Visualizations"), (pngfiles, "PNG")):
            print(
                "Could not find any {} files.".format(kind)
                if len(files) == 0
    ...
    if verbose > 0:
        print(
            "Found {} python file{} and {} visualization app{}".format(
                len(pyfiles),
                "" if len(pyfiles) == 1 else "s",
                len(visualizations),
                "" if len(visualizations) == 1 else "s",
            )
        )
        if verbose > 1:
        ...
        with open(model, "r") as modelFile:
            for line in modelFile:
                if line.startswith("if __name__ == '__main__':"):
                    print(
                        "import {}".format(
                            ", ".join(viz.__module__ for viz in visualizations)
                        ),
                        file=appFile,
                    )
                    print(
                        "showVisualizations(({}), version={})".format(
                            ", ".join(
    ...
    if verbose > 0:
        print(
            "Created application file with {} visualiazation{} in {}".format(
                len(visualizations),
                "" if len(visualizations) == 1 else "s",
                appFilename,
            )
