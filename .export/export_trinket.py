...
replacements = [("▶", "=left_arrow="), ("▢", "W"), ("✓", "X"), ("ø", "!"), ("∞", "inf")]
...
def export_trinket(
    ...
    if not all(os.path.exists(os.path.join(source_directory, f)) for f in mustHave):
        msg = (
            "Missing some files.  Expected to find: {}\n"
            "Is the source directory, {}, correct?"
        ).format(", ".join(mustHave), source_directory)
        raise Exception(msg)
    ...
    if os.path.exists(outdir):
        if os.path.isdir(outdir) and force:
            ...
            print(
                "{} exists.  Use --force option to clear contents first".format(outdir)
            )
            sys.exit(0)
        else:
            print(
    ...
    for filename in files:
        ...
        if any(regex.match(basename) for regex in exclude):
            if verbose > 0:
                print("Excluding {} for match to an exclude pattern".format(basename))
            continue
            ...
            with open(filename, "r") as infile:
                ...
                if verbose > 0:
                    print(
                        "Exporting {}{}{}".format(
                            basename,
                            " with substitions" if content != revised else "",
                            (
                                " ignoring lingering non-ASCII characters"
                                if containsNonASCII and ignore
                                else ""
                            ),
                        )
                    )
