...
class Table(list):  # Display a table (array/list) in a visualization app
    ...
    def __init__(  # Constructor
        ...
        *args: "Initial elements in table (not used by Table)",
        ...
        if labelAnchor is None:
            labelAnchor = (
                (
                    (N if direction < 0 else S)
                    if vertical
                    else (W if direction < 0 else E)
                )
                if labelPosition is None
                else (
                    S
                    if labelPosition is N
                    else N if labelPosition is S else W if labelPosition is E else E
                )
            )
        if labelPosition is None:
            labelPosition = (
                (S if direction < 0 else N) if vertical else (E if direction < 0 else W)
            )
        self.labelAnchor = labelAnchor
        self.labelPosition = labelPosition
        self.labelOffset = labelOffset
        self.labelFont = app.VARIABLE_FONT if labelFont is None else labelFont
        self.labelColor = app.VARIABLE_COLOR if labelColor is None else labelColor
        ...
        self.labeledArrowFont = (
            app.VARIABLE_FONT if labeledArrowFont is None else labeledArrowFont
        )
        self.labeledArrowColor = (
            app.VARIABLE_COLOR if labeledArrowColor is None else labeledArrowColor
        )
    ...
    def drawCells(self, see=(), scale=None):
        ...
        self.cells = [  # Create any needed new cells
            (
                ...
                if (
                    j < len(self.cells)
                    and self.app.canvas.type(self.cells[j]) == "rectangle"
                )
                else self.app.canvas.create_rectangle(
                    *self.arrayCellCoords(j, scale=scale),
                    fill="",
                    outline=self.cellBorderColor,
        ...
        if self.indicesFont and self.indicesColor:  # Create any needed indices
            self.indices = [
                (
                    ...
                    if (
                        j < len(self.indices)
                        and self.app.canvas.type(self.indices[j]) == "text"
                    )
                    else self.app.canvas.create_text(
                        *self.arrayCellIndexCoords(j, scale=scale),
                        text=str(j),
                        anchor=self.indicesAnchor,
                        font=self.indicesFont,
                        fill=self.indicesColor,
                        tags=self.indicesTags
                    )
                )
                for j in range(len(self))
            ]
        ...
        for textItem in self.indices:
            for event, handler in self.eventHandlers:
                if (
                    callable(handler)
                    and hasattr(Table, handler.__name__)
    ...
    def cellCoords(self, indexOrCoords, scale=None):
        if scale is None:
            scale = getattr(self.app, "scale", 1)
        ...
        upperLeft = (
            ...
            if coordsGiven
                ...
                self.VOrigin
                + V(V((1, 0) if self.vertical else (0, 1)) * (row * self.segmentGap))
            )
            + V(self.VBox * V(0 if self.vertical else col, col if self.vertical else 0))
        )
        return (
            V(
                upperLeft
    ...
    def cellAndCenters(self, indexOrCoords, rows=1, scale=None):
        ...
        textCenters = tuple(
            Vcenter + V(Vy * (j - rows / 2 + 1 / 2)) for j in range(max(1, rows))
        )
    ...
    def labeledArrowCoords(
        ...
        gap = (-scale if level < 0 else scale) * (
            self.cellBorderWidth
            + self.labeledArrowOffset
            + (abs(self.indicesFont[1]) if self.indicesFont and level > 0 else 0)
        )
        side = 2 if level < 0 else 0
        tip = (
            (cell[side] - gap, center[1])
            if self.vertical
            else (center[0], cell[side + 1] - gap)
        )
        Vdelta = V(
    ...
    def labelCoords(self, scale=None):
        ...
        gap = self.labelOffset * (scale if self.labelPosition in (N, W) else -scale)
        vertical = self.labelPosition in (N, S)
        return (
            center[0] if vertical else cell0[side] - gap,
    ...
    def createLabeledArrow(
        ...
        **kwargs
        ...
        if color is None:
            color = self.labeledArrowColor
        if font is None:
            font = self.labeledArrowFont
        coords = (
            self.labeledArrowCoords(labeledArrowIndexOrCoords, scale=scale, **kwargs)
            if isinstance(labeledArrowIndexOrCoords, (int, float))
            else labeledArrowIndexOrCoords
        )
        if anchor is None:
        ...
        text = self.app.canvas.create_text(
            *coords[1], anchor=anchor, text=label, font=font, fill=color, tags=tags
        )
    ...
    def populateArgWithCellIndexHandler(self, argIdx=0):
        def cellIndexHandler(event):
            ...
            if event.widget is can:
                for item in can.find_closest(
                    can.canvasx(event.x), can.canvasy(event.y)
                ):
...
if __name__ == "__main__":
    ...
    table1 = Table(
        ...
        indicesAnchor=W,
        indicesOffset=10,
        eventHandlerPairs=(("<Button-1>", Table.populateArgWithCellIndexHandler),),
    )
    ...
    print(
        "Walking labeled arrow from", interval[0], "to", interval[1], "of", table2.label
    )
    j = interval[0]
    ...
    for j in range(j + 1, interval[1] + 1):
        ...
        app.moveItemsTo(
            jArrow,
            table2.labeledArrowCoords(j, **jArrowConfig),
            sleepTime=0.02,
            see=True,
            expand=True,
        )
    ...
    while table3[0].val > 15:
        ...
        app.moveItemsBy(table3[0].items, (-3 * len(table3), -table3.y0), sleepTime=0.02)
        app.moveItemsTo(
            flat(*(dv.items for dv in table3[1:])),
            flat(
                *(
                    (table3.cellCoords(j), table3.cellCenter(j))
    ...
    print(
        'Table for Boolean values "{}"'.format(table4.label),
        "with label position",
        table4.labelPosition.upper(),
        "and label anchor",
        table4.labelAnchor.upper(),
    )
    ...
    while j < len(table3):
        if 0 <= j and isinstance(table3[j].val, bool):
            boolToMove = table3[j]
            print("Deleting cell", j, "of table", table3.label, "item", boolToMove.val)
            app.moveItemsBy(
                boolToMove.items,
            ...
            app.moveItemsLinearly(
                boolToMove.items,
                table4.cellAndCenters(len(table4)),
                sleepTime=0.02,
                see=True,
                expand=True,
            )
            ...
            app.moveItemsTo(
                jArrow,
                table3.labeledArrowCoords(j, **jArrowConfig),
                sleepTime=0.02,
                see=True,
                expand=True,
            )
