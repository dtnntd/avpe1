...
"""
    ...
    def delete(self, goal, code=deleteCode, start=True):
        ...
        if self.highlightCode(
            "node is not None", callEnviron, wait=wait, returnValue=self.getNode(node)
        ):
            self.highlightCode("return self.__delete(parent, node)", callEnviron)
...
"""
    ...
    def __delete(self, parent, node, code=__deleteCode, level=1):
        ...
        deletedLabel = self.canvas.create_text(
            *(V(upperRightNodeCoords) - V(self.CIRCLE_SIZE + 5, 0)),
            anchor=E,
            text="deleted",
        ...
        self.moveItemsTo(
            deletedKeyAndData,
            self.nodeItemCoords(upperRightNodeCoords)[1:],
            sleepTime=wait / 10,
        )
        ...
        if self.getLeftChild(node):
            ...
            if self.getRightChild(node):
                ...
                self.highlightCode(
                    "self.__promote_successor(node)", callEnviron, wait=wait
                )
                localVars = (deletedLabel, *deletedKeyAndData, *parentIndex, *nodeIndex)
                colors = self.canvas.fadeItems(localVars)
                self.__promote_successor(node)
                self.canvas.restoreItems(localVars, colors)
                ...
                if parent == -1:
                    self.highlightCode("self.__root = node.leftChild", callEnviron)
                    self.replaceSubtree(
                        parent, Child.RIGHT, self.getLeftChildIndex(node), callEnviron
                    )

                else:
                    self.highlightCode(
                        "parent.leftChild is node", callEnviron, wait=wait
                    )
                    ...
                    self.replaceSubtree(
                        parent,
                        Child.LEFT if nodeIsLeft else Child.RIGHT,
                        self.getLeftChildIndex(node),
                        callEnviron,
                    )
        else:
            self.highlightCode(("parent is self", 2), callEnviron, wait=wait)
            if parent == -1:
                self.highlightCode("self.__root = node.rightChild", callEnviron)
                rightChildIndex = self.getRightChildIndex(node)
                rightChild = self.getNode(rightChildIndex)
                self.replaceSubtree(parent, Child.RIGHT, rightChildIndex, callEnviron)
                if rightChild is None:
                    self.updateTreeObjectRootPointer(root=self.getRoot())

            else:
                self.highlightCode(
                    ("parent.leftChild is node", 2), callEnviron, wait=wait
                )
                nodeIsLeft = self.getLeftChildIndex(parent) == node
                self.highlightCode(
                    (
...
"""
    ...
    def __promote_successor(self, nodeIndex, code=__promote_successorCode):
        wait = 0.1
        ...
        self.highlightCode("successor = node.rightChild", callEnviron, wait=wait)
        ...
        while self.getLeftChild(successor):
            ...
            self.highlightCode(
                "successor = successor.leftChild", callEnviron, wait=wait
            )
        ...
        successorDataItem, successorKeyItem = tuple(
            self.canvas.copyItem(item) for item in successorNode.drawnValue.items[1:]
        )
        ...
        self.moveItemsTo(
            successorDataItem,
            self.canvas.coords(node.drawnValue.items[1]),
            sleepTime=wait / 10,
        )
        self.canvas.copyItemAttributes(
    ...
    def clickDelete(self):
        ...
        if val is not None:
            ...
            msg = (
                "Deleted {}".format(val)
                if deleted
                else "Value {} not found".format(val)
            )
            self.setMessage(msg)
            self.clearArgument()
...
if __name__ == "__main__":
    ...
    tree = BinaryTree(values=[int(a) for a in nonneg] if len(nonneg) > 0 else None)
