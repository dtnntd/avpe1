...
def raiseAppWindowHandler(
    appWindow, menubutton, menu, adjustForTrinket=False, sticky=(N, E, W, S)
):
    def handler():
...
def makeArmMenuHandler(menubar, menu):
    def handler(event):
        setattr(
            ...
            "timeout_ID",
            menubar.after(
                HOVER_DELAY, lambda: menu.grid() or setattr(menubar, "timeout_ID", None)
            ),
        )
...
def resizeHandler(event):
    ...
    if kind == "top" and event.width > 1 and event.height > 1:
        ...
        if DEBUG:
            print("Scheduling resize of window leaving", topPartHeight, "at top")
        event.widget.after(
            50,
            lambda: event.widget.columnconfigure(0, minsize=event.width)
            or event.widget.rowconfigure(1, minsize=event.height - topPartHeight),
        )
...
def showVisualizations(  # Display a set of VisualizationApps in pulldown menu
    ...
    if theme:
        if debug:
            print(
                "Starting theme:", ttkStyle.theme_use(), "among", ttkStyle.theme_names()
            )
    ...
    nextline = makeIntro(
        intro_msg.format(
            customInstructions=(
                trinketInstructions if adjustForTrinket else desktopInstructions
            ).strip()
        ),
        intro,
    ...
    loading = ttk.Label(
        intro, text="\nLoading ...", font=INTRO_FONT + ("italic",), style=labelStyleName
    )
    ...
    menu.add_command(
        label="Introduction",
        command=raiseAppWindowHandler(
            intro, menubutton, menu, adjustForTrinket, sticky=()
        ),
    ...
    folders = {}
    for folder, apps in PREFERRED_ARRANGEMENT:
        folders[folder] = [classes_dict[app] for app in apps if app in classes_dict]
    otherApps = [
        app
        for app in classes
        if not any(app.__name__ in apps for folder, apps in PREFERRED_ARRANGEMENT)
    ]
    if otherApps:
    ...
    for folder in folders:
        ...
        for app in folders[folder]:
            if verbose > 0:
                print(
                    "Found app {} and instantiating in {}".format(app.__name__, folder),
                    file=sys.stderr,
                )
            pane = ttk.Frame(top)
            ...
            menu.add_command(
                label=name,
                font=MENU_FONT + (chosenStyle if startAppWindow == pane else ()),
                command=raiseAppWindowHandler(pane, menubutton, menu, adjustForTrinket),
            )

    for i, appWindow in enumerate(appWindows[1:]):
        if (
            appWindow is not (userSelectedWindow or startAppWindow)
            and appWindow.winfo_ismapped()
        ):
    ...
    if version:
        if isinstance(version, str):  # Hidden handler to show version info
            for button in range(1, 4):
                intro.bind(
                    "<Double-Button-{}>".format(button),
                    lambda event: (
                        event.state & CTRL
                        or (isinstance(event.num, int) and event.num != 1)
                    )
                    and loading.configure(text=version),
                )
        elif isinstance(version, (tuple, list)):
            spacer = ttk.Label(intro, text=" ", font=INTRO_FONT, style=labelStyleName)
            version = ttk.Label(
                intro,
            ...
            raise ValueError(
                "Version must be string or tuple/list, not {}".format(type(version))
            )
    ...
    if verbose > 1:
        print(
            ...
            top.winfo_geometry(),
            "Menubutton geometry:",
            menubutton.winfo_geometry(),
            "RestoreMenu geometry:",
            restoreMenu.winfo_geometry(),
            "RestoreMenu height:",
            restoreMenu["height"],
            file=sys.stderr,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="Adjust settings and warn users about limitations of use on " "Trinket.io",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--seed",
        default="3.14159",
        help="Random number generator seed.  Set to empty string to skip " "seeding.",
    )
    parser.add_argument(
        "-d",
        "--debug",
    ...
    if args.files is None or args.files == []:
        ...
        if sys.argv and os.path.exists(sys.argv[0]) and os.path.dirname(sys.argv[0]):
        ...
        if args.verbose > 1:
            print(
                "No files provided.  Unique directories to search:",
                dirs,
                "with search order:",
                list(dirs),
            )
    ...
    if args.version and args.version.startswith("(") and args.version.endswith(")"):
        args.version = eval(args.version)
    showVisualizations(
        findVisualizations(args.files, args.verbose),
        start=args.start,
        title=args.title,
