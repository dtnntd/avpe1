    ...
    for fileOrDir in filesAndDirectories:
        ...
        if verbose > 1:
            print(
                ...
                (
                    "{} python files in {}".format(len(files), fileOrDir)
                    if isDir
                    else fileOrDir
                ),
                file=sys.stderr,
            ...
            if modulename:
                try:
                    ...
                    if verbose > 1:
                        if addPath:
                            print(
                                "Adding {} to sys.path ...".format(path),
                                file=sys.stderr,
                            )
                        print(
                            "Attempting to import {} ... ".format(modulename),
                            file=sys.stderr,
                            end="",
                        )
                    if addPath:
                    ...
                    if verbose > 1:
                        print(
                            "Imported. Looking for VisualizationApp".format(modulename),
                            file=sys.stderr,
                        )
                    newclasses = findVisualizationClasses(module, verbose)
                    if verbose > 1:
                        print(
                            "Found {} matching classes: {}".format(
                                len(newclasses), newclasses
                            ),
                            file=sys.stderr,
                        )
                        previouslyFound = set(newclasses) & classes
                        if len(previouslyFound) > 0:
                            print("Previously found:", previouslyFound, file=sys.stderr)
                    classes |= set(newclasses)
                except ModuleNotFoundError:
                    if verbose > 0:
                        print("Unable to import module", modulename, file=sys.stderr)
...
def isPatternInFile(textOrRegex, filename):
    with open(filename, "r", encoding="utf-8") as f:
        return (
            (text in f.read())
            if isinstance(textOrRegex, str)
            else (
                textOrRegex.search(f.read())
                if isinstance(textOrRegex, type(pathsep))
                else False
            )
        )
...
def findVisualizationClasses(module, verbose=0):
    ...
    for name in dir(module):
        ...
        if (
            isinstance(this, type(object))  # is a class that is a proper
            and issubclass(this, VAPs)
            and not (this in VAPs)  # subclass of VAP
            and this.__module__ == module.__name__
        ):  # defined in ths module
            classes.append(this)
        elif verbose > 2:
            print(
                "Ignoring {}.{} of type {}".format(module.__name__, name, type(this)),
                file=sys.stderr,
            )
    return classes
...
def makeIntro(
    ...
    for line in mesg.split("\n"):
        URLs = [m for m in URL_pattern.finditer(line)] if URLfg and URLfont else []
        if URLs:
            ...
            if last < len(line):
                label = ttk.Label(
                    URLframe, text=line[last:], font=font, style=styleName
                )
                label.grid(row=0, column=col)
                col += 1
...
def oneTimeShowHintHandler(visualizationApp, hintText=None, delay=500):
    ...
    return lambda event: (
        getattr(visualizationApp, "initial_hint_shown", False)
        or visualizationApp.window.after(
            delay,
            lambda: visualizationApp.setHint(visualizationApp.textEntries[0], hintText),
        )
        and setattr(visualizationApp, "initial_hint_shown", True)
    )
