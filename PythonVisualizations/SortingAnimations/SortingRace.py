...
class Car(object):
    ...
    def move(self, message, totalInversions):  # moves car based on inversions removed
        ...
        Draw.moveTo(
            self.__draw, self.__x + (totalInversions - message[2]) * step, self.__y
        )
...
def race(a, cars):
    ...
    totalInversions = bubbleInversions = selectionInversions = insertionInversions = (
        mergeInversions
    ) = quickInversions = countInversions(a)
...
def main():
    ...
    cars = {
        "bubble": bubbleCar,
        "selection": selectionCar,
        "insertion": insertionCar,
        "merge": mergeCar,
        "quick": quickCar,
    }
