...
def animateArrayAction(
    ...
    if action == shuffle:
        ...
        p = multiprocessing.Process(
            target=action, args=(q, "bubbleList", bubbleList, 250)
        )
        jobs.append(p)
        # take action on middle-left array
        p = multiprocessing.Process(
            target=action, args=(q, "selectionList", selectionList, 250)
        )
        jobs.append(p)
        # take action on bottom-left array
        p = multiprocessing.Process(
            target=action, args=(q, "insertionList", insertionList, 250)
        )
        jobs.append(p)
        # take action on top-right array
        p = multiprocessing.Process(
            target=action, args=(q, "cocktailList", cocktailList, 250)
        )
        jobs.append(p)
        ...
        p = multiprocessing.Process(
            target=action, args=(q, "shellList", shellList, 250)
        )
        jobs.append(p)
        ...
        # take action on middle-left array
        p = multiprocessing.Process(
            target=action, args=(q, "selectionList", selectionList)
        )
        jobs.append(p)
        # take action on bottom-left array
        p = multiprocessing.Process(
            target=action, args=(q, "insertionList", insertionList)
        )
        jobs.append(p)
        # take action on top-right array
        p = multiprocessing.Process(
            target=action, args=(q, "cocktailList", cocktailList)
        )
        jobs.append(p)
        # take action on middle-right array
...
def animateDrawArray(
    q, bubbleList, selectionList, insertionList, cocktailList, combList, shellList
):
    # list of processes
    jobs = []
    ...
    p = multiprocessing.Process(
        target=createArray, args=(q, "bubbleList", bubbleList, 25, 50, 20)
    )
    jobs.append(p)
    # create middle-left array
    p = multiprocessing.Process(
        target=createArray, args=(q, "selectionList", selectionList, 25, 300, 20)
    )
    jobs.append(p)
    # create bottom-left array
    p = multiprocessing.Process(
        target=createArray, args=(q, "insertionList", insertionList, 25, 550, 20)
    )
    jobs.append(p)
    # create top-right array
    p = multiprocessing.Process(
        target=createArray, args=(q, "cocktailList", cocktailList, 575, 50, 20)
    )
    jobs.append(p)
    # create middle-right array
    p = multiprocessing.Process(
        target=createArray, args=(q, "combList", combList, 575, 300, 20)
    )
    jobs.append(p)
    # create bottom-right array
    p = multiprocessing.Process(
        target=createArray, args=(q, "shellList", shellList, 575, 550, 20)
    )
    jobs.append(p)
    ...
    while numDone < len(jobs):
        ...
        if message[1] == "done":  # Message format is [name, 'done']
            ...
            if message[0] == "bubbleList":
                drawArray(
                    bubbleList, message[1], message[2], message[3], message[4], 20
                )
            elif message[0] == "selectionList":
                drawArray(
                    selectionList, message[1], message[2], message[3], message[4], 20
                )
            elif message[0] == "insertionList":
                drawArray(
                    insertionList, message[1], message[2], message[3], message[4], 20
                )
            elif message[0] == "cocktailList":
                drawArray(
                    cocktailList, message[1], message[2], message[3], message[4], 20
                )
            elif message[0] == "combList":
