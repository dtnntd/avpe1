...
def setCanvasSize(w=_DEFAULT_CANVAS_SIZE, h=_DEFAULT_CANVAS_SIZE):
    ...
    _canvas = tkinter.Canvas(_tkWindow, width=w, height=h, bd=0, highlightthickness=0)
    _canvas.pack()
...
def rect(x, y, width, height):
    ...
    id = _canvas.create_rectangle(
        x, y, x + width - 1, y + height - 1, outline=_penColor, width=1
    )
...
def filledRect(x, y, width, height):
    ...
    id = _canvas.create_rectangle(x, y, x + width, y + height, fill=_penColor, width=0)
...
def _makeIterable(a):
    t = type(a)
    if not (t is list or t is tuple):
        a = (a,)
    return a
...
def mouseLeft():
    ...
    raise Exception("Can't determine mouse button if click hasn't happened")
