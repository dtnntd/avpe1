...
class Sorter(object):
    ...
    def __init__(self, name, row, col):
        ...
        self.__arrow1 = (
            ...
            Draw.filledRect(
                1000 + round(max(5, BOX / BARS) / 3),
                1000,
                round(max(5, BOX / BARS) / 3),
                bodyLength,
            ),
        )
        self.__arrow2 = (
    ...
    def sortProcess(self, array):
        ...
        self.__processID = multiprocessing.Process(
            ...
            args=(Sorter.__q, self.__array, self.__name, self.__row),
        )
...
def rowHeader():
    ...
    for i in range(len(shuffles) // 2):
        Draw.string(
            shuffles[i * 2],
            8,
            (i + 1) * BOX // 3 + (Y * 8 * i + Y) - 2 * (BOX // 3) // 3,
        )
        Draw.string(
            shuffles[i * 2 + 1],
            8,
            (i + 1) * BOX // 3 + (Y * 8 * i + Y) - (BOX // 3) // 2,
        )
...
def speed(speedScale, x, y, speedValue):
    ...
    if (
        ...
        Draw.moveTo(
            speedScale[-1],
            speedScale[0] + int(speedScale[0] / 100 * speedValue) - speedScale[4] / 2,
            speedScale[3],
        )
    ...
    while not Sorter.allDone():
        ...
        if Draw.mousePressed():
            if Draw.mouseLeft():
                ...
                speedValue = speed(speedScale, clickedX, clickedY, speedValue)
