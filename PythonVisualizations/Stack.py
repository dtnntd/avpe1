...
class Stack(VisualizationApp):
    ...
    def __init__(  # Constructor
        ...
        **kwargs
        ...
        self.CELL_WIDTH = textWidth(self.VALUE_FONT, "▢" * (maxArgWidth + 2))
        self.size = size
        self.list = []
    ...
    def createIndex(self, index, name="__top"):
        ...
        if name:
            label = self.canvas.create_text(
                arrowCoords[0] - abs(self.VARIABLE_FONT[1]),
                arrowCoords[1],
                text=name,
                anchor=E,
...
"""
    ...
    def pop(self, code=popCode, start=True):
        ...
        labelPos = ((itemPos[0] + itemPos[2]) / 2, itemPos[1] - self.CELL_HEIGHT / 2)

        topLabel = self.canvas.create_text(
            *labelPos, text="top", font=self.VARIABLE_FONT, fill=self.VARIABLE_COLOR
        )
        callEnviron.add(topLabel)

        self.moveItemsTo(
            newItems,
            (itemPos, (labelPos[0], itemPos[1] + self.CELL_HEIGHT // 2)),
            sleepTime=wait / 5,
        )
        ...
        self.highlightCode("self.__top -= 1", callEnviron)
        self.moveItemsBy(self.topIndex, (0, (self.CELL_HEIGHT)), sleepTime=wait / 5)
...
"""
    ...
    def outputBoxCoords(self):
        x0, y0 = self.cell0()
        return (
            x0 + self.CELL_WIDTH * 3 // 2,
            y0 - 30,
            x0 + self.CELL_WIDTH * 5 // 2,
            y0,
        )

    def topBoxCoords(self):
        outputBox = self.outputBoxCoords()
        return (outputBox[0], self.CELL_HEIGHT * 2, outputBox[2], self.CELL_HEIGHT * 3)
...
def isEmpty(self):
    ...
    def isEmpty(self, code=isEmptyCode):
        ...
        callEnviron |= set(self.createIndex(-0.5, name="0"))
    ...
    def cellCoords(self, cell_index):  # Get bounding rectangle for array cell
        ...
        return (
            x0,
            y0 - self.CELL_HEIGHT * (cell_index + 1),
    ...
    def createArrayCell(self, index):  # Create a box representing an array cell
        ...
        cell = V(cell_coords) + V(-half_border, -half_border, other_half, other_half)
        rect = self.canvas.create_rectangle(
            cell, fill=None, outline=self.CELL_BORDER_COLOR, width=self.CELL_BORDER
        )
        self.canvas.lower(rect)
        label = self.canvas.create_text(
            cell[0] - abs(self.numericIndexFont[1]),
            (cell[1] + cell[3]) // 2,
            text=str(index),
            fill=self.numericIndexColor,
            font=self.numericIndexFont,
        )
...
"""
    ...
    def display(self, newSize=None, code=newCode, start=True):
        ...
        if newSize is not None:
            ...
            try:
                ...
                wait = 0
            self.size = newSize
            self.CELL_HEIGHT = min(
                self.MAX_CELL_HEIGHT, canvasDims[1] // max(6, newSize + 2)
            )
            self.VALUE_FONT = (
                self.VALUE_FONT[0],
    ...
    def fixCells(self):
        if self.topIndex:
            ...
            if len(self.topIndex) > 1:
                self.canvas.coords(
                    self.topIndex[1], coords[0] - abs(self.VARIABLE_FONT[1]), coords[1]
                )
    ...
    def makeButtons(self, maxRows=4):
        width_vcmd = (self.window.register(makeWidthValidate(self.maxArgWidth)), "%P")
        self.pushButton = self.addOperation(
            "Push",
            lambda: self.clickPush(),
            numArguments=1,
            argHelpText=["item"],
            validationCmd=width_vcmd,
            helpText="Push item on stack",
        )
        newStackButton = self.addOperation(
            "New",
            lambda: self.clickNewStack(),
            numArguments=1,
        ...
        popButton = self.addOperation(
            "Pop",
            lambda: self.clickPop(),
            helpText="Pop item from stack",
            maxRows=maxRows,
        )
        peekButton = self.addOperation(
            "Peek",
            lambda: self.clickPeek(),
            helpText="Peek at top stack item",
            maxRows=maxRows,
        )
    ...
    def clickPeek(self):
        ...
        self.setMessage(
            "Value {} is at the top of the stack!".format(val)
            if val
    ...
    def clickNewStack(self):
        ...
        cell0 = self.cell0()
        maxCells = 10
        while int((cell0[1] // (maxCells + 2)) * self.FONT_RATIO) > self.MIN_FONT_SIZE:
            maxCells += 1
