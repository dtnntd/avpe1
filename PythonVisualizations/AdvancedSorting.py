...
"""
    ...
    def medianOfThree(self, lo, hi, code=medianOfThreeCode):
        ...
        self.highlightCode(
            "key(self.get(lo)) > key(self.get(mid))", callEnviron, wait=wait
        )
        ...
        self.highlightCode(
            "key(self.get(lo)) > key(self.get(hi))", callEnviron, wait=wait
        )
        ...
        self.highlightCode(
            "key(self.get(hi)) > key(self.get(mid))", callEnviron, wait=wait
        )
...
"""
    ...
    def partition(self, pivot, lo, hi, code=__partCode):
        ...
        while lo <= hi:
            self.highlightCode("key(self.get(lo)) < pivot", callEnviron, wait=wait)
            ...
            if lo < hi or self.useMedianOf3.get():
                self.highlightCode("pivot < key(self.get(hi))", callEnviron, wait=wait)
            while (self.useMedianOf3.get() or lo < hi) and (pivot < self.list[hi].val):
                ...
                if lo < hi or self.useMedianOf3.get():
                    self.highlightCode(
                        "pivot < key(self.get(hi))", callEnviron, wait=wait
                    )
...
"""
    ...
    def quicksort(
        ...
        codeWait = 0.01
        callEnviron = self.createCallEnvironment(
            code=code.format(**locals()), sleepTime=codeWait, startAnimations=start
        )
        ...
        if hi - lo + 1 <= short:
            self.highlightCode(
                "return self.insertionSort(lo, hi, key)", callEnviron, wait=wait
            )
        ...
        self.highlightCode(
            "pivotItem = self.{}(lo, hi, key)".format(
        ...
        colors = self.canvas.fadeItems(loIndex + hiIndex)
        pivotItem = (
            self.medianOfThree(lo, hi)
            if self.useMedianOf3.get()
            else self.rightmost(lo, hi)
        )
        self.canvas.restoreItems(loIndex + hiIndex, colors)
        ...
        self.highlightCode("self.quicksort(lo, hipart - 1, short, key)", callEnviron)
        colors = self.canvas.fadeItems(loIndex + hiIndex + hipartIndex)
        self.quicksort(lo, hipart - 1, short, code, bottomCallEnviron=bottomCallEnviron)
        self.canvas.restoreItems(loIndex + hiIndex + hipartIndex, colors)
        self.canvas.tag_raise("pivotPartition")

        self.highlightCode("self.quicksort(hipart + 1, hi, short, key)", callEnviron)
        colors = self.canvas.fadeItems(loIndex + hiIndex + hipartIndex)
        self.quicksort(hipart + 1, hi, short, code, bottomCallEnviron=bottomCallEnviron)
        self.canvas.restoreItems(loIndex + hiIndex + hipartIndex, colors)
        self.canvas.tag_raise("pivotPartition")
    ...
    def showPartitionsControlCoords(self):
        ...
        return (gap, canvasDims[1] - gap - size, gap + size, canvasDims[1] - gap)
    ...
    def createShowPartitionsControl(self):
        ...
        self.showPartitionsCheck = self.canvas.create_text(
            *(center if self.showPartitions else V(center) * -1),
            text="✓",
            fill=self.CONTROLS_COLOR,
            font=self.CONTROLS_FONT
        )
        label = self.canvas.create_text(
            *(V(center) + V(coords[2] - coords[0], 0)),
            anchor=W,
            text="Show pivot partitions",
            fill=self.CONTROLS_COLOR,
            font=self.CONTROLS_FONT
        )
    ...
    def toggleShowPartitons(self, event=None):
        ...
        for item in self.canvas.find_withtag("pivotPartition") + (
            ...
            self.canvas.coords(item, *(sign * abs(c) for c in self.canvas.coords(item)))
    ...
    def insertionSort(self, lo, hi):
        ...
        while outer <= hi:
            ...
            while inner > lo and temp < self.list[inner - 1].val:
                ...
                if deltaX != 0:
                    self.moveItemsBy(
                        innerIndex + tempVal.items + (label,),
                        (deltaX, 0),
                        sleepTime=wait / 5,
                    )
            ...
            # Advance outer loop
            outer += 1
            self.moveItemsBy(outerIndex, (self.CELL_WIDTH, 0), sleepTime=wait / 10)
...
def shellSort(self):
    ...
    def shellSort(self, code=shellSortCode, start=True):
        ...
        nShiftsLabel = self.canvas.create_text(
            *nShiftsPos,
            anchor=W,
            text=nShiftsValue.format(nShifts),
        ...
        while h > 0:
            ...
            self.highlightCode("outer in range(h, len(self))", callEnviron, wait=wait)
            for outer in range(h, len(self.list)):
                ...
                if outerCoords != arrowPos:
                    self.moveItemsTo(
                        outerArrow, (arrowPos, arrowPos[:2]), sleepTime=moveWait
                    )
                ...
                itemsToMove = list(
                    flat(*[self.list[j].items + (self.arrayCells[j],) for j in toMove])
                )
                toMoveCoords = [
                    V(self.canvas.coords(i)) + V(descend) for i in itemsToMove
                ]
                itemsToRestore = list(
                    flat(
                ...
                toRestoreCoords = list(
                    flat(
                        *[
                            [self.fillCoords(self.list[j].val, self.cellCoords(j))]
                            + (
                                [self.cellCenter(j)]
                                if len(self.list[j].items) > 1
                                else []
                            )
                            + [self.arrayCellCoords(j)]
                            for j in toRestore
                        ]
                    )
                )
                ...
                if innerArrow is None:
                    ...
                    self.moveItemsTo(
                        innerArrow, (arrowPos, arrowPos[:2]), sleepTime=moveWait
                    )

                self.highlightCode("inner >= h", callEnviron, wait=wait)
                if inner >= h:
                    self.highlightCode(
                        "temp < self.get(inner-h)", callEnviron, wait=wait
                    )

                while inner >= h and tempVal.val < self.list[inner - h].val:
                    self.highlightCode(
                        "self.set(inner, self.get(inner-h))", callEnviron
                    )
                    self.assignElement(
                        inner - h,
                        inner,
                    ...
                    tempCoords = [
                        V(self.canvas.coords(i)) + V(delta * 2) for i in tempVal.items
                    ] + [tempLabelPos]
                    self.moveItemsTo(
                ...
                if inner < outer:
                    ...
                    self.assignFromTemp(
                        inner, tempVal, tempLabel, delete=False, sleepTime=moveWait
                    )

                    # Take temp out of the cleanup set since it should persist
                    callEnviron -= set(tempVal.items)

                    self.highlightCode(("nShifts += 1", 2), callEnviron, wait=wait)
                    nShifts += 1
                    self.canvas.itemConfig(
                        nShiftsLabel, text=nShiftsValue.format(nShifts)
                    )
                ...
                self.highlightCode(
                    "outer in range(h, len(self))", callEnviron, wait=wait
                )
    ...
    def adjustHLabel(
        self, h, leftCell, hLabel, steps=10, sleepTime=0.01, font=None, color=None
    ):
        ...
        if hLabel:  # Adjust existing hLabel: (span, box, text)
            ...
            if currentCoords != spanCoords or currentText != hText:
                for step in range(steps):
                    sCoords = (
                        V(
                            V(V(currentCoords) * (steps - (step + 1)))
                            + V(V(spanCoords) * (step + 1))
                        )
                        / steps
                    )
                    ...
                    if startH != h:
                        text = "h: {:3.1f}".format(
                            (startH * (steps - 1 - step) + h * (step + 1)) / steps
                        )
                        tWidth = textWidth(font, text)
                    bCoords = (
                        sCenter[0] - tWidth / 2 - 1,
                        sCoords[1],
                        sCenter[0] + tWidth / 2 + 1,
                        sCoords[3],
                    )
                ...
                for i, coords in zip(hLabel, (spanCoords, boxCoords, spanCenter)):
                    self.canvas.coords(i, *coords)
            ...
            box = self.canvas.create_rectangle(
                *boxCoords, fill=self.DEFAULT_BG, width=0, outline=""
            )
    ...
    def makeButtons(self, maxRows=4):
        ...
        partitionButton = self.addOperation(
            "Partition",
            lambda: self.clickPartition(),
            numArguments=1,
            validationCmd=vcmd,
            maxRows=maxRows,
            argHelpText=["item key"],
            helpText="Partition array around key",
        )
        ...
        useMedianOf3Button = self.addOperation(
            "Use median of 3",
            self.clickUseMedianOf3,
            buttonType=Checkbutton,
            variable=self.useMedianOf3,
            maxRows=maxRows,
    ...
    def clickPartition(self):
        ...
        if val is None:
            ...
            self.setMessage(
                "Partition at or above {}\nstarts at index {}".format(val, part)
            )
    ...
    def clickQuicksort(self):
        ...
        self.quicksort(
            code=(
                self.quicksortCode
                if self.useMedianOf3.get()
                else self.quicksortCode.replace("medianOfThree", "rightmost").replace(
                    "lo + 1,", "lo,"
                )
            ),
...
if __name__ == "__main__":
    ...
    array = AdvancedArraySort(values=[int(a) for a in nonneg] if nonneg else None)
