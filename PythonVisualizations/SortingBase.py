...
class SortingBase(VisualizationApp):
    ...
    def assignElement(
        ...
        toValue = self.list[toIndex] if toIndex < len(self.list) else drawnValue(0)

        # create new display objects as copies of the "from" cell and value
        copyItems = [self.canvas.copyItem(i) for i in fromValue.items if i is not None]
        callEnviron |= set(copyItems)

        # get positions of "to" cell in array
        toPositions = (
            self.fillCoords(fromValue.val, self.currentCellCoords(toIndex)),
            self.currentCellCenter(toIndex),
        )[: len(copyItems)]

        # Move copies to the desired location
        if startAngle == 0:  # move items linearly
            self.moveItemsTo(copyItems, toPositions, steps=steps, sleepTime=sleepTime)
    ...
    def tempCoords(self, index):  # Determine coordinates for a temporary var
        ...
        bottom = min(canvasDims[1] - 1, cellCoords[3] + int(self.CELL_HEIGHT * 1.7))
    ...
    def assignToTemp(
        ...
        if existing:
            ...
            tempLabelPos = tempCoords if tempCoords else self.tempLabelCoords(index)
        ...
        newCoords = [V(self.canvas.coords(i)) + V(delta * 2) for i in copyItems] + [
            tempLabelPos
        ]
        self.moveItemsTo(copyItems + [tempLabel], newCoords, sleepTime=sleepTime)

        return drawnValue(fromValue.val, *copyItems), tempLabel

    def assignFromTemp(self, index, temp, templabel, delete=True, sleepTime=0.04):
        ...
        self.moveItemsOnCurve(
            temp.items,
            (toCellCoords, toCellCenter)[: len(temp.items)],
            sleepTime=sleepTime,
            startAngle=startAngle,
        )
        ...
        self.moveItemsOnCurve(
            ...
            [
                (
                    coordsA
                    if i == A.items[0]
                    else coordsB if i == B.items[0] else self.canvas.coords(i)
                )
                for i in itemsB + itemsA
                if i is not None
            ],
            sleepTime=0.05,
            startAngle=90 * 11 / (10 + abs(a - b)),
        )
    ...
    def computeCellWidth(self, nCells=None):
        ...
        return max(
            self.CELL_MIN_WIDTH,
            min(self.CELL_SIZE, (canvasDims[0] - self.ARRAY_X0) // (nCells + 1)),
        )

    def canShowValues(self, width=None):
        if width is None:
            width = self.CELL_WIDTH
        return (self.CELL_BORDER + textWidth(self.VALUE_FONT, str(self.valMax))) < width
...
"""
    ...
    def new(self, val, code=newCode, start=True):
        canvasDims = widgetDimensions(self.canvas)
        maxCells = min(
            self.maxCells, (canvasDims[0] - self.ARRAY_X0) // self.CELL_MIN_WIDTH - 1
        )
        if val > maxCells:
            self.setMessage("Too many cells; must be {} or less".format(maxCells))
            return
        elif val < 1:
        ...
        self.size = val
        self.highlightCode("self.__a = [None] * initialSize", callEnviron, wait=0.1)
        self.list = []
...
"""
    ...
    def insert(self, val, allowGrowth=False, code=insertCode, start=True):
        ...
        if (
            len(self.list) >= self.size
            and not allowGrowth
            or len(self.list) >= self.maxCells
            or self.CELL_WIDTH == self.CELL_MIN_WIDTH
            and offCanvas
        ):
        ...
        toPositions = (
            self.fillCoords(val, self.cellCoords(len(self.list))),
    ...
    def randomFill(self):
        ...
        if toFill > 0:
            self.list.extend(
                [drawnValue(random.randrange(self.valMax)) for i in range(toFill)]
            )
        else:
        ...
        if toFill > 0:
            self.list.extend(
                [
                    drawnValue(
                        int(
                            (i if increasing else max(1, toFill - 1) - i)
                            * self.valMax
                            / max(1, toFill - 1)
                        )
                    )
    ...
    def delete(self, val, code=deleteCode, start=True):
        ...
        for j, n in enumerate(self.list):
            ...
            if n.val == val:
                ...
                self.moveItemsOffCanvas(n.items + (foundCircle,), N, sleepTime=0.02)

                self.highlightCode(
                    "k in range(j, self.__nItems)", callEnviron, wait=0.1
                )
                ...
                for k in range(j, len(self.list) - 1):
                    self.highlightCode("self.__a[k] = self.__a[k+1]", callEnviron)
            ...
            self.moveItemsBy(Jindex, (self.CELL_WIDTH, 0), sleepTime=0.01)
...
"""
    ...
    def traverse(self, code=traverseCode, start=True):
        ...
        outputFont = (self.VALUE_FONT[0], int(self.VALUE_FONT[1] * 0.75))
        ...
        for j, n in enumerate(self.list):
            ...
            valueOutput = (
                ...
                if len(n.items) > 1 and n.items[1]
            ...
            toPositions = (
                outputBoxCoords[0] + padding / 2 + (j + 1 / 2) * spacing,
                midOutputBox,
            )
            self.moveItemsTo((valueOutput,), (toPositions,), sleepTime=0.02)
    ...
    def outputBoxCoords(self, outputFont, padding=10, N=None):
        ...
        return (
            left,
            canvasDims[1] - abs(outputFont[1]) * 3 - padding,
            left + N * spacing + padding,
            canvasDims[1] - padding,
        )

    def isSorted(self):
        return all(self.list[i - 1] <= self.list[i] for i in range(1, len(self.list)))

    def cellCoords(self, index):  # Get bounding rectangle for indexed array cell
        return (
            self.ARRAY_X0 + self.CELL_WIDTH * index,
            self.ARRAY_Y0,
    ...
    def currentCellCoords(self, index):
        ...
        return V(
            V(self.canvas.coords(self.arrayCells[closest]))
            + V(((index - closest) * self.CELL_WIDTH, 0) * 2)
        ) - V(self.arrayCellDelta())
    ...
    def createCellValue(self, indexOrCoords, key, color=None):
        ...
        cell = (
            self.canvas.create_rectangle(*newCoords, fill=color, outline="", width=0),
        )
        if self.showValues:
            cell += (
    ...
    def display(self, showNItems=True):
        ...
        self.nItems = (
            self.createIndex(len(self.list), "nItems", level=-1, color="black")
            if showNItems
            else None
        )
    ...
    def toTarget(  # Return the 2-D vector between the indexed item's
        self, index
    ):  # current position and its normal position in array
    ...
    def shuffle(self, steps=20):
        ...
        velocity = [  # Initial velocity is mostly down and towards destination
            (
                (
                    random.randint(int(dx / 3) - 1, 1)
                    if dx < 0
                    else random.randint(-1, int(dx / 3) + 1)
                ),
    ...
    def fixCells(self):  # Move canvas display items to exact cell coords
        ...
        for i, dValue in enumerate(self.list):
            for item, coords in zip(
                dValue.items,
                (self.fillCoords(dValue.val, self.cellCoords(i)), self.cellCenter(i)),
            ):
                if item is not None:
    ...
    def makeButtons(self, maxRows=4):
        ...
        insertButton = self.addOperation(
            "Insert",
            lambda: self.clickInsert(),
            numArguments=1,
            validationCmd=vcmd,
            maxRows=maxRows,
            argHelpText=["item key"],
            helpText="Insert item in array",
        )
        searchButton = self.addOperation(
            "Search",
            lambda: self.clickSearch(),
            numArguments=1,
            validationCmd=vcmd,
            maxRows=maxRows,
            argHelpText=["item key"],
            helpText="Search for item in array",
        )
        deleteButton = self.addOperation(
            "Delete",
            lambda: self.clickDelete(),
            numArguments=1,
            validationCmd=vcmd,
            maxRows=maxRows,
            argHelpText=["item key"],
            helpText="Delete array item",
        )
        newButton = self.addOperation(
            "New",
            lambda: self.clickNew(),
            numArguments=1,
            validationCmd=vcmd,
            maxRows=maxRows,
    ...
    def clickSearch(self):
        ...
        if val is None:
            ...
            self.setMessage("Value {} {}found".format(val, "" if found else "not "))
            self.clearArgument()
    ...
    def clickDelete(self):
        ...
        if val is None:
            ...
            msg = "Value {} {}".format(val, "deleted" if result else "not found")
            self.setMessage(msg)
        self.clearArgument()
