...
def BBoxesOverlap(bbox1, bbox2, zeroOK=True):
    ...
    return all(
        rangesOverlap(
            bbox1[j], bbox1[j + half], bbox2[j], bbox2[j + half], zeroOK=zeroOK
        )
        for j in range(half)
    )
...
def BBoxContains(bbox1, bboxOrPoints):
    ...
    half = len(bbox1) // 2
    return all(
        bbox1[j] <= bboxOrPoints[j + k] and bboxOrPoints[j + k] <= bbox1[j + half]
        for j in range(half)
        for k in range(0, len(bboxOrPoints), half)
    )
...
def BBoxIntersection(*bboxes):
    ...
    if len(bboxes) == 0:
        ...
        half = len(bboxes[0]) // 2
        return tuple(
            (max if j < half else min)(bbox[j] for bbox in bboxes)
...
def BBoxUnion(*bboxes):
    ...
    if len(bboxes) == 0:
        ...
        half = len(bboxes[0]) // 2
        return tuple(
            (min if j < half else max)(bbox[j] for bbox in bboxes)
            for j in range(len(bboxes[0]))
        )
...
def BBoxEnclosing(*coordinates, dims=2):
    ...
    minCoords = tuple(
        min(coordinates[j] for j in range(dim, len(coordinates), dims))
        for dim in range(dims)
    )
    maxCoords = tuple(
        max(coordinates[j] for j in range(dim, len(coordinates), dims))
        for dim in range(dims)
    )
...
def widgetDimensions(widget):  # Get widget's (width, height)
    ...
    return (
        (int(widget.config("width")[-1]), int(widget.config("height")[-1]))
        if geom[0] == 1 and geom[1] == 1
        else geom[:2]
    )
...
def widgetGeometry(widget, geom=None):
    ...
    if parsed.group(3) == "-" or parsed.group(5) == "-":
        nums = nums[:2] + (
            widget.winfo_screenwidth() - nums[2] if parsed.group(3) == "-" else nums[2],
            (
                widget.winfo_screenheight() - nums[3]
                if parsed.group(5) == "-"
                else nums[3]
            ),
        )
...
def widgetState(widget, state=None):  # Get or set widget state
    if isinstance(widget, (ttk.Button,)):
        if state is None:
            ...
            widget.state(("!disabled", "!pressed") if state == NORMAL else (state,))
    else:
        if state is None:
            return widget["state"]
        else:
...
def tkFontFromSpec(spec):
    ...
    size = (
        ...
        if (
            len(spec) > 1
            and (
                isinstance(spec[1], int)
                or (isinstance(spec[1], str) and sizePattern.match(spec[1]))
            )
        )
        else 0
    )
...
def genericEventHandler(
    ...
    def genericHandler(event):
        if to:
            print(
                "{} event on {}:".format(event.type, event.widget),
                ", ".join(
                    "{} = {!r}".format(attr, getattr(event, attr)) for attr in fields
                ),
                file=to,
            )
            if event.state and isinstance(event.state, int):
                print(
                    "Modifiers: {}".format(
                        ", ".join(
                            [mod.name for mod in Modifier if event.state & mod.value]
                        )
                    ),
                    file=to,
                )
...
class Scrim(Canvas):
    ...
    def copyItemAttributes(  # Copy attributes from one canvas item to another
        ...
        kwargs = dict(
            (attrib, self.itemConfig(fromItem, attrib)) for attrib in attributes
        )
    ...
    def copyItem(self, canvasitem, includeBindings=True):
        ...
        creator = getattr(
            self,  # Get canvas creation function for type
            "create_{}".format(self.type(canvasitem)),
        )
        newItem = creator(*self.coords(canvasitem), **self.itemConfig(canvasitem))
        ...
        for item in items:
            if self.type(item) == "text":
                ...
                if anchor in self.anchorVectors and anchor != newAnchor:
                    ...
                    delta = (
                        V(
                            V(self.anchorVectors[newAnchor])
                            - V(self.anchorVectors[anchor])
                        )
                        / 2
                    )
    ...
    def bbox(
        self, tagOrID: "Tag or ID of item for bounding box" = None
            ...
            if kind == "text":
                ...
                upperLeft = tuple(
                    map(
                        int,
                        V(coords - halfSize)
                        - V(halfSize * V(self.anchorVectors[anchor])),
                    )
                )
    ...
    FADED_COLORS = {
        ...
        "outline": "bisque",
        "activefill": "bisque",
        "activeoutline": "bisque",
        "disabledfill": "gray90",
        "disabledoutline": "gray90",
    }
    ...
    def getItemColors(self, item):
        ...
        return dict([(key, config[key]) for key in self.TYPE_COLORS[self.type(item)]])

    def getItemFont(self, item):
        return parseTkFont(self.itemConfig(item, "font"))

    def create_hashed_rectangle(self, x0, y0, x1, y1, angle=45, spacing=10, **kwargs):
        ...
        points = [
            (BBox[i], BBox[j]) for i, j in ((0, 1), (0, 3), (2, 3), (2, 1), (0, 1))
        ]
        ...
        for a in range(steps):
            ...
            p1 = (
                min(BBox[2], max(BBox[0], xlo if odd else xhi)),
                min(BBox[3], max(BBox[1], ylo if odd else yhi)),
            )
            p2 = (
                min(BBox[2], max(BBox[0], xhi if odd else xlo)),
...
def getPhotoImage(filename, size, cache=True, path=None):
    ...
    if not cache or (id(image), size) not in __tk_image_cache__["PhotoImage"]:
        ...
        if __tk_image_cache__["debug"]:
            print(
                "Resizing {} into PhotoImage cache with ratio {}".format(
                    filename, ratio
                ),
                end=" ",
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-v", "--verbose", action="count", default=0, help="Add verbose comments"
    )
    ...
    for dims in range(2, 4):
        ...
        for i in range(len(bboxes) - 1):
            ...
            print("BBox {} {} empty".format(bboxes[i], "is" if boxiEmpty else "is not"))
            for j in range(i + 1, len(bboxes)):
                print("BBox 1 = {!s:25}  BBox 2 = {!s:25}".format(bboxes[i], bboxes[j]))
                for f in (BBoxUnion, BBoxIntersection, BBoxesOverlap, BBoxContains):
                    print(
                        "{}({}, {}) = {}".format(
                            f.__name__, bboxes[i], bboxes[j], f(bboxes[i], bboxes[j])
                        )
                    )
                print(
                    "{}({}, {}) = {}".format(
                ...
                if not boxiEmpty and BBoxEmpty(BBoxIntersection(bboxes[i], bboxes[j])):
                    print("The first box is not empty but the intesection is.")
                if not all(
                    BBoxContains(BBoxUnion(bboxes[i], bboxes[j]), bb)
                    for bb in (bboxes[i], bboxes[j])
                ):
                    print(
                        "Union of {} {} does not contain both!".format(
                            bboxes[i], bboxes[j]
                        )
                    )
                if not all(
                    BBoxContains(bb, BBoxIntersection(bboxes[i], bboxes[j]))
                    for bb in (bboxes[i], bboxes[j])
                ):
                    print(
                        (
                            "The bounding boxes {} {} do not both contain"
                            "their intersection!"
                        ).format(bboxes[i], bboxes[j])
                    )
                if not all(
                    BBoxContains(bb, BBoxIntersection(bboxes[i], bboxes[j]) * 2)
                    for bb in (bboxes[i], bboxes[j])
                ):
                    print(
                        (
    ...
    def compare_bbox(
        ...
        details: "True -> show all config details, or dict of keys to show" = None,
    ):
        ...
        print(
            "After hiding its bounding box is",
            hiddenBBox,
            "which is",
        ...
        print(
            "After hiding its coordinates are",
            hiddenCoords,
            "which are",
    ...
    print("The default font for canvas text items is:", scrim.getItemFont(buttonState))
    ...
    tkButton["command"] = lambda: (
        widgetState(tkButton, DISABLED)
        or widgetState(ttkButton, NORMAL)
        or updateButtonState()
    )
    ttkButton["command"] = lambda: (
        widgetState(tkButton, NORMAL)
        or widgetState(ttkButton, DISABLED)
        or updateButtonState()
    )
    ...
    if pngFiles:
        ...
        images = [getPhotoImage(pngFile, (height, height)) for pngFile in pngFiles]
    ...
    def updateGeometry(event=None):
        scrim.itemConfig(
            geometry, text="widgetGeometry(tk) = {}".format(widgetGeometry(tk))
        )
    ...
    print("The font for the group details text item is:", scrim.getItemFont(message))
    ...
    def groupButtonHandler(btn, tag):
        def groupButtonPress():
            ...
            btn["text"] = " ".join((states[(sIndex + 1) % len(states)], *parts[1:]))
            if state == "Fade":
    ...
    def hashedRectRotator(
        rect, bbox, angle=10, spacing=10, deltaAngle=10, deltaSpacing=0
    ):
    ...
    tags = []
    shapeCreators = [scrim.create_arc, scrim.create_oval, scrim.create_hashed_rectangle]

    for x0, y0 in ((50, 20), (100, 90), (180, 110), (240, 110), (300, 130), (380, 150)):
        g = len(tags)
        tag = "group{:02d}".format(g)
        shape = shapeCreators[g % len(shapeCreators)]
        kwargs = (
            {"start": 30, "extent": 200}
            if shape == scrim.create_arc
            else (
        ...
        shapeItem = shape(
            *bbox,
            fill=palette[(g + 0) % nc],
            activefill=palette[(g + 1) % nc],
            disabledfill=palette[(g + 2) % nc],
            outline=palette[(g + 3) % nc],
            activeoutline=palette[(g + 4) % nc],
        ...
        if shape == scrim.create_hashed_rectangle:
            scrim.tag_bind(
                shapeItem, "<Button-1>", hashedRectRotator(shapeItem, bbox, **kwargs)
            )
