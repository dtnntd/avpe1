...
class TowerOfHanoi(VisualizationApp):
    ...
    def display(self):
        ...
        starOnAxis = regularStar((self.bbox[2] - 5 * sw, 0), sw * 2, sw * 2 * 0.6, 6)
        tiltStar = [
            V(V(vert) / V(1, 3)) + V(0, baseBottom - sw * 2) for vert in starOnAxis
        ]
        self.base = (
...
"""
    ...
    def setupDisks(self, nDisks, code=_init__Code, start=True):
        ...
        for spindle in range(3):
            ...
            if callEnviron:
                ...
                self.moveItemsTo(
                    spindleLabel,
                    self.spindleLabelCenter(spindle + 1),
                    sleepTime=wait / 5,
                )
    ...
    def createSpindleDrawing(self, index):
        ...
        items = (
            self.canvas.create_oval(
                *coords[0], fill=self.spindleSide, outline="", width=0, tags=tags
            ),
            self.canvas.create_rectangle(
                *coords[1], fill=self.spindleSide, outline="", width=0, tags=tags
            ),
            self.canvas.create_oval(
                *coords[2], fill=self.spindleTop, outline="", width=0, tags=tags
            ),
        )
    ...
    def spindleCoords(self, index):
        ...
        return (  # Bottom disk
            (
                self.spindleX(index) - half,
                bottom - quarter,
                self.spindleX(index) + (sw - half),
                bottom + (half - quarter),
            ),
            # Rectangle shaft
            (
    ...
    def createDiskDrawing(self, diskID):
        ...
        startMoveHandler = self.startMoveHandler(diskID, tags[1], normal, highlight)
    ...
    def isPickable(self, diskID):
        spindle, pos = self.diskSpindlePos(diskID)
        return spindle is None or (
            self.picked is None and self.spindles[spindle][-1] == diskID
        )

    def isPlacable(self, diskID, spindle):
        return spindle is not None and (
            len(self.spindles[spindle]) == 0 or self.spindles[spindle][-1] > diskID
        )

    def isDone(self):
        return (
            len(self.spindles[0]) == 0
            and len(self.spindles[1]) == 0
            and len(self.spindles[2]) == len(self.disks)
            and len(self.spindles[2]) > 0
        )
        ...
        def leaveEnterHandler(event):
            kwargs = (
                ...
                if (
                    ...
                    and self.isPickable(ID)
                    and not self.operationMutex.locked()
                )
                else normal
            )
    ...
    def moveHandler(self, ID, tag, normal, highlight):
        def mvHandler(event):
            if self.picked is None or not self.operationMutex.locked():
                return
            self.canvas.move(tag, event.x - self.lastPos[0], event.y - self.lastPos[1])
            self.lastPos = (event.x, event.y)
            diskBBox = self.canvas.bbox(tag)
            for spindle, spindleBBox in enumerate(self.spindleBBoxes):
                kwargs = (
                    highlight
                    if BBoxesOverlap(diskBBox, spindleBBox)
                    and self.isPlacable(ID, spindle)
                    else normal
                )
    ...
    def releaseHandler(self, ID, tag, normal, highlight):
        def relHandler(event):
            ...
            dropAt = None
            for spindle, spindleBBox in enumerate(self.spindleBBoxes):
                if BBoxesOverlap(diskBBox, spindleBBox) and self.isPlacable(
                    ID, spindle
                ):
                    dropAt = spindle
                    self.moves += 1
            ...
            self.restoreTo(
                diskItems,
                diskCoords,
                after=lambda items, coords: [
                    self.canvas.coords(i, c) for i, c in zip(items, coords)
                ],
            )
    ...
    def updateSpindles(self, *spindleIDs):
        for spindle in spindleIDs if spindleIDs else range(3):
            ...
            self.spindleBBoxes[spindle] = self.canvas.bbox(self.spindleTag(spindle))
            self.window.update()
    ...
    def showCompletion(self):
        ...
        star = self.canvas.create_polygon(
            *starCoords, fill="goldenrod", outline="red", width=max(1, size / 100)
        )
        ...
        while size < targetSize or starCenter != targetCenter:
            starCenter = (  # Move star towards target and increase size
                targetCenter
                if V(V(targetCenter) - V(starCenter)).len2() < 2
                else (V(V(targetCenter) * (1 - rate)) + V(V(starCenter) * rate))
            )
            size = (
                targetSize
                if abs(targetSize - size) < 2
                else (targetSize * (1 - rate) + size * rate)
            )
            angle = (
                targetAngle
                if abs(targetSize - size) < 2
                else (targetAngle * (1 - rate) + angle * rate)
            )
            starCoords = regularStar(starCenter, size, size * ratio, points, angle)
        ...
        callEnviron.add(
            self.canvas.create_text(
                *starCenter,
                text="in {} move{}!".format(self.moves, "" if self.moves == 1 else "s"),
                font=font,
                anchor=S
            )
        )
    ...
    def diskTag(self, ID):
        return "disk {}".format(ID)

    def spindleTag(self, ID):
        return "spindle {}".format(ID)
...
"""
    ...
    def solve(self, nDisks=None, start=0, goal=2, spare=1, startAnimations=True):
        if nDisks is None:
            nDisks = self.nDisks()
        highlightWait = 0.08
        moveWait = 0.01
        ...
        self.highlightCode(
            "nDisks={nDisks}, start={start}, goal={goal}, spare={spare}".format(
                **locals()
            ),
            callEnviron,
        )
        ...
        for label, pos in labelPositions:
            if (
                label not in self.spindleLabels
                or len(self.canvas.coords(self.spindleLabels[label])) == 0
            ):
        ...
        labelCoords = [self.canvas.coords(labelItem) for labelItem in labelItems]
        ...
        self.highlightCode("self.solve(nDisks - 1, start, spare, goal)", callEnviron)
        self.solve(nDisks - 1, start, spare, goal)
        self.moveItemsTo(labelItems, toCoords, sleepTime=moveWait)
        self.canvas.itemconfigure(labelItems[0], text=startLabel)
        ...
        self.moves += 1
        self.highlightCode("self.solve(nDisks - 1, spare, goal, start)", callEnviron)
        self.solve(nDisks - 1, spare, goal, start)
        self.moveItemsTo(labelItems, toCoords, sleepTime=moveWait)
        self.canvas.itemconfigure(labelItems[0], text=startLabel)
    ...
    def moveDisk(self, fromSpindle, toSpindle):
        ...
        self.moveItemsBy(
            ...
            (
                0,
                min(fromSpindleTopBBox[1], fromSpindleTopBBox[3])
                - max(diskBBox[1], diskBBox[3]),
            ),
            sleepTime=wait,
        )
        delta = V((toSpindleTopBBox[0] - fromSpindleTopBBox[0], 0) * 2)
        toCoords = [delta + V(self.canvas.coords(item)) for item in self.disks[ID]]
        ...
        self.moveItemsTo(self.disks[ID], self.diskCoords(ID), sleepTime=wait)
        self.canvas.tag_lower(tag, "spindle")
        self.updateSpindles(toSpindle)

    def restoreTo(
        self, items, targetCoords, by=0.2, dist2=4, wait=30, increaseBy=1.1, after=None
    ):
        ...
        targets = [
            ...
            for item, tgt in zip(items, targetCoords)
        ...
        for item, coords, target in targets:
            self.canvas.coords(item, V(V(coords) * (1 - by)) + V(V(target) * by))
        self.canvas.after(
            wait,
            lambda: self.restoreTo(
                items, targetCoords, by * increaseBy, dist2, wait, increaseBy, after
            ),
        )
    ...
    def enableButtons(self, enable=True):  # Customize enableButtons
        ...
        self.solveButton["state"] = (  # more diskks
            NORMAL if enable and self.nDisks() and self.leftSpindleFull() else DISABLED
        )
    ...
    def clickNew(self):
        ...
        if val is None:
            self.setMessage(
                "Input must be an integer from {} to {}".format(
                    self.minDisks, self.maxDisks
                )
            )
        else:
            nDisks = int(val)
            ...
            self.moves = 0
            self.setMessage(
                "You need at least {} move{}.  Good luck!".format(
    ...
    def clickSolve(self):
        if self.leftSpindleFull():
            ...
            self.setMessage(
                "{} total move{} made".format(
                    self.moves, "" if self.moves == 1 else "s"
                )
            )
        else:
            self.setMessage(
                "All {} disks must be on left spindle to solve".format(self.nDisks())
            )
