...
class vector(object):
    ...
    def __init__(self, *coords):
        self.coords = (
            tuple(coords[0])
            if (len(coords) == 1 and isinstance(coords[0], (list, tuple, set)))
            else coords
        )
...
def convexPolygon(center, radius, nGon, startAngle=90):
    V = vector
    return tuple(
        V(center) + V(radius, 0).rotate(startAngle + j * 360 / nGon)
        for j in range(nGon)
    )
...
def regularStar(center, outerRadius, innerRadius, nPoints, startAngle=90):
    ...
    return tuple(
        V(center)
        + V(outerRadius if j % 2 == 0 else innerRadius, 0).rotate(
...
if __name__ == "__main__":
    ...
    expressions = [
        ...
        "V(1e-1, 1e-10).unit()",
        "V(1e-10, 0).unit(minLength=1e-9)",
        "V(1e-10, 0).unit(minLength=1e-10)",
        "V(1e-10, 0).unit(minLength=1e-11)",
        "bbox(A, B)",
        "bbox(B[1:], A)",
        "flat(*bbox(A, B))",
        "bbox(A[1:], B[1:])",
