...
class drawnValue(object):
    ...
    def __init__(  # Constructor
        ...
        *items
    ):  # List of canvas items depicting the value
    ...
    def __getitem__(self, key):  # Implement positIonal access
        if isinstance(key, int):
            ...
            return [
                self[k]
                for k in range(
                    key.start or 0, key.stop or 1 + len(self.items), key.step or 1
                )
            ]
    ...
    def __getattr__(self, name):
        ...
        raise AttributeError("drawnValue has no attribute {}".format(repr(name)))
    ...
    def __setattr__(self, name, val):
        ...
        if name in self.__legacy_fields:
            ...
            self.items = tuple(val if i == pos else v for i, v in enumerate(self.items))
            return
        raise AttributeError("drawnValue has no attribute {} to set".format(repr(name)))
...
if __name__ == "__main__":
    ...
    print(
        "Colors of first", row, "drawn values:", [i.color(canvas) for i in items[:row]]
    )
    ...
    print("Geting the first sorted drawn value's display_shape:", first.display_shape)
    print(
        "The first drawn value's index 1 is:",
        first[1],
        "and its display_val is:",
        first.display_val,
    )
