...
def gridDict(frame):  # Get all widget's within a frame's grid indexed by
    ...
    return defaultdict(
        lambda: None,
        zip(
            [(int(s.grid_info()["column"]), int(s.grid_info()["row"])) for s in slaves],
            slaves,
        ),
    )
...
def makeFilterValidate(maxWidth, exclude=""):
    ...
    return lambda value_if_allowed: (
        len(value_if_allowed) <= maxWidth
        and all(c not in exclude for c in value_if_allowed)
    )
...
class VisualizationApp(Visualization):  # Base class for visualization apps
    ...
    def setUpControlPanel(self):  # Set up control panel structure
        ...
        self.message = Label(
            ...
            font=self.CONTROLS_FONT + ("italic",),
            fg="blue",
            bg=self.DEFAULT_BG,
        )
    ...
    def newValueCoords(self, buffer=30, offCanvas=False):
        ...
        maxY = (
            self.canvasBounds[3]
            if self.canvasBounds and offCanvas
            else visibleCanvas[3]
        )
    ...
    def getOperations(self):
        ...
        for item in self.operations.grid_slaves():
            if (
                isinstance(item, self.buttonTypes)
                and getattr(item, "required_args", 0) > 0
            ):
    ...
    def addOperation(  # Add a button to the operations control panel
        ...
        **kwargs
        ...
        if buttonType in (ttk.Button,):
            ttk.Style().configure("TButton", font=self.CONTROLS_FONT, background=bg)
            button = buttonType(self.operations, text=label, **kwargs)
        else:
            button = buttonType(  # Create button based on type
                self.operations, text=label, font=self.CONTROLS_FONT, bg=bg, **kwargs
            )
        button["command"] = self.runOperation(callback, cleanUpBefore, button, mutex)
        ...
        if numArguments:
            ...
            button.grid(column=buttonColumn, row=buttonRow, padx=8, sticky=(E, W))
            ...
            for i, entry in enumerate(self.textEntries):
                ...
                entryRowSpan = (
                    ...
                    if i < nEntries - 1
                        ...
                        1, min(maxRows, len(withArgs)) - 1 - (nEntries - 1) * rowSpan
                    )
                )
        ...
        if helpText:
            button.bind(
                "<Enter>", self.makeArmHintHandler(button, helpText)
            )  # equivalent of ttp 'enter'
            button.bind(
                "<Leave>", self.makeDisarmHandler(button)
            )  # equivalent of ttp 'close'
    ...
    def createArgumentEntry(self, validationCmd):
        ...
        entry.bind("<KeyRelease>", lambda ev: self.argumentChanged(ev.widget), "+")
        for key in ("Return", "KP_Enter"):
            entry.bind(
                "<KeyPress-{}>".format(key), lambda ev: self.returnPressed(ev), "+"
            )
    ...
    def makeArmHintHandler(self, widget, helpText=None):
        def handler(event):
            hint = (
                " or ".join(getattr(widget, "helpTexts", set()))
                if helpText is None
                else helpText
            )
            setattr(
                ...
                widget.after(
                    self.HOVER_DELAY,
                    lambda: self.setHint(widget, hint)
                    or setattr(widget, "timeout_ID", None),
                ),
            )
    ...
    def recordModifierKeyState(self, event=None):
        if event and event.type in (EventType.ButtonPress, EventType.KeyPress):
            ...
            self.debugRequested = event.state & (CTRL | SHIFT) == (
                CTRL | SHIFT
            ) and os.path.exists(".debug.pyc")

    def startMode(self):
        "Choose starting animation mode based on last modifier keys used"
        return Animation.STEP if self.modifierKeyState & SHIFT else Animation.RUNNING

    def returnPressed(self, event):  # Handle press of Return/Enter in text
        button = getattr(event.widget, "last_button", None)  # entry argument widget.
    ...
    def pressButton(self, button):  # Simulate button press, if enabled
        if widgetState(button) == NORMAL:
            ...
            time.sleep(0.05)
            widgetState(
                button, "!" + PRESSED if isinstance(button, ttk.Button) else NORMAL
            )
            button.invoke()
    ...
    def addAnimationButtons(self, maxRows=4, setDefaultButton=True):
        ...
        withArgs, withoutArgs = self.getOperations()
        lastRow, lastColumn = (
            self.getOperationGridLocation(withoutArgs[0])
            if withoutArgs
            else (1, self.withoutArgsColumn)
        )
        ...
        self.pauseButton, self.stepButton, self.stopButton = (
            Button(
                self.playControlsFrame,
                image=self.playControlImages[name],
                state=DISABLED,
            )
    ...
    def runOperation(self, command, cleanUpBefore, button=None, mutex=True):
        def animatedOperation():  # If button that uses arguments is provided,
            if button and getattr(button, "required_args", 0) > 0:  # record it
                for entry in self.textEntries[
                    : getattr(button, "required_args")  # as the last button
                ]:  # pressed for all its args
                    setattr(entry, "last_button", button)
            animationControls = [
                b for b in (self.pauseButton, self.stepButton, self.stopButton) if b
            ]
            withArgs, withoutArgs = self.getOperations()
            try:
                if cleanUpBefore:
                ...
                self.cleanUp(
                    self.callStack[0] if self.callStack else None, ignoreStops=True
                )
            ...
            if (
                focus  # If focus ended on a button needing arguments or an
                and (
                    focus in withArgs  # animation control run on something w/
                    or focus in animationControls
                    and button in withArgs
                )  # args
                and self.textEntries
            ):  # and there are text entry boxes
                self.textEntries[0].focus_set()  # switch focus to 1st entry
            elif (
                focus  # If focus ended on animation control run on
                and focus in animationControls  # something without args
                and button in withoutArgs
            ):
    ...
    def getArguments(self, clear=False):
        return [self.getArgument(i, clear=clear) for i in range(len(self.textEntries))]
    ...
    def showCode(
        self,  # Show algorithm code in a scrollable text box
        ...
        allowStepping=True,
    ):  # Allow stepping on waits
        code = code.strip()
        if len(code) == 0:  # Empty code string?
            return  # then nothing to show
        if self.codeText is None:
            padX, padY = 10, 10
            self.codeTextCharWidth = textWidth(self.CODE_FONT, "0123456789") // 10
            self.codeVScroll = Scrollbar(self.codeFrame, orient=VERTICAL)
            self.vScrollWidth = max(self.vScrollWidth, self.codeVScroll.winfo_width())
            width = self.codeTextWidth(padX, self.vScrollWidth)
            self.codeText = Text(
                self.codeFrame,
                wrap=NONE,
                background=self.OPERATIONS_BG,
                font=self.CODE_FONT,
                width=width,
                height=self.MIN_CODE_CHARACTER_HEIGHT,
                padx=padX,
                pady=padY,
                takefocus=False,
            )
        ...
        if addBoundary and currentCode and not currentCode.isspace():
            self.codeText.insert("1.0", self.codeText.config("width")[-1] * "-" + "\n")
    ...
    def removeCode(
        self,  # Remove algorithm code from the codeText box
        code,  # Code to remove
        ...
        allowSteps=False,
    ):  # Allow step mode pauses before and during
        ...
        if allowSteps:
            try:
                ...
                inUserStop = True
        last_line = int(float(self.codeText.index(END)) if len(lines) > 0 else 0)
        for i in range(1, min(last_line, len(lines) + (2 if boundary else 1))):
            if self.codeText:
                self.codeText.delete("1.0", "2.0")
                if sleepTime > 0 and not self.animationsStopped() and not inUserStop:
                    try:
    ...
    def codeTextWidth(  # Compute width available for code text
        ...
        debug=False,
    ):  # Set true for debugging printout
        ...
        if mainWidth < max(upperWidth, lowerWidth) or mainWidth == 1:
            ...
            lowerWidth = 500
        available = mainWidth - max(upperWidth, lowerWidth) - vScrollWidth - padX * 2
        desired = min(
            80, max(self.MIN_CODE_CHARACTER_WIDTH, available // self.codeTextCharWidth)
        )
        if debug:  # Set true for debugging printout
            print(
                "Call to codeTextWidth, codeText =",
                self.codeText,
                "which {} mapped".format(
                    "is"
                ...
                "available pixels =",
                available,
                "codeText character width =",
                self.codeTextCharWidth,
    ...
    def resizeCodeText(self, event=None, debug=False):
        ...
        if debug:
            print(
                "Entering resizeCodeText with event =",
                event,
                "self.codeText = ",
                ct,
                "{} mapped".format("is" if ct.winfo_ismapped() else "is not"),
                "timeout_ID =",
                getattr(ct, resize_ID, None),
            )
        ...
        if debug and False:  # Set true for debugging printout
            ...
            if skip and desired != nCharsWide:
                print("Skipping resize while timer {} running".format(timeout_ID))
    ...
    def highlightCode(
        ...
        if isinstance(fragments, (list, tuple)):
            if (
                len(fragments) == 2  # Look for (str, int) pair
                and isinstance(fragments[0], (str, type(geomPattern)))
                and isinstance(fragments[1], int)
            ):
                frags = [tuple(fragments)]  # Look up by (str, int) tuple
            else:
                frags = [
                    tuple(frag) if isinstance(frag, (list, tuple)) else (frag, 1)
        ...
        for tagName in self.codeText.tag_names() if self.codeText else []:
            ...
            self.codeText.tag_config(
                tagName,
                background=color if highlight else "",
        ...
        if not found and len(tags) > 0:  # This shouldn't happen so log bug
            print(
                "Unable to find highlight tag(s) {} among {}".format(
    ...
    def cleanUp(
        self,  # Remove Tk items from past animations either
        ...
        allowSteps=False,
    ):  # Allow stepping to pause clean up
        if stopAnimations:
            self.stopAnimations()
    ...
    def cleanUpCallEnviron(  # Clean up a call on the stack
        self,
        callEnviron,  # removing the call environement
        sleepTime=0,  # waiting sleepTime between removing code lines
        allowSteps=False,
    ):  # allowing step pauses if set
        inUserStop = False
        codeBlock = None
        toDelete = []
    ...
    def createCallEnvironment(self, code="", sleepTime=0, startAnimations=True):
        ...
        if startAnimations:
            self.startAnimations(
                state=(
                    startAnimations
                    if isinstance(startAnimations, Animation)
                    else (
                        self.animationState
                        if startAnimations is True and not self.animationsStopped()
    ...
    def yieldCallEnvironment(
        self,
        callEnviron: "Call environment to remove from stack",
        ...
        if callEnviron is not self.callStack[-1]:
            raise Exception("Cannot yield from call environment that is not current")
        codeBlock = self.getCodeHighlightBlock(callEnviron)
        if codeBlock:
        ...
        itemCoords = {}
        canvasDims = (
            V(self.canvasBounds[2:]) - self.canvasBounds[:2]
            if self.canvasBounds
            else widgetDimensions(self.canvas)
        )
        away = V(canvasDims) * 10
        itemStackingOrder = self.canvas.find_all()
        for item in callEnviron:
            if isinstance(item, int) and self.canvas.type(item) and not item in exclude:
                itemIndex = itemStackingOrder.index(item)
                if moveItems:
                    coords = self.canvas.coords(item)
                    if any(
                        self.withinCanvas((coords[j], coords[j + 1]))
                        for j in range(0, len(coords), 2)
                    ):
                        itemCoords[item] = (coords, itemIndex)
                        self.canvas.coords(
                            item, V(coords) + V(away * (len(coords) // 2))
                        )
                else:
    ...
    def resumeCallEnvironment(
        self,
        callEnviron: "Call environment to push back on stack",
    ...
    def speed(self, sleepTime):
        return min(10, sleepTime * 50 * self.SPEED_SCALE_MIN / self.speedScale.get())
    ...
    def wait(self, sleepTime, allowStepping=True):
        ...
        if (
            self.animationsStepping()
            and buttonImage(self.pauseButton) != self.playControlImages["play"]
        ):
            buttonImage(self.pauseButton, self.playControlImages["play"])
        highlights = self.callStackHighlights()
        if (
            allowStepping
            and self.animationsStepping()
            and self.lastHighlights != highlights
        ):
            if len(highlights) > 0 and highlights[-1]:
                ...
                for fragment in reversed(highlights[-1]):
                    for index in reversed(
                        self.codeText.tag_ranges(codeBlock[fragment])
                    ):
                        self.codeText.see(index)
    ...
    def buttonFocus(self, btn, hasFocus):
        ...
        def focusHandler(event=None):
            ...
            btn["relief"] = "groove" if hasFocus else "raised"
            ...
            if isinstance(btn, self.buttonTypes):
                ...
                widgetState(
                    ...
                    (
                        ...
                        if enable
                        and all(args[:nArgs])
                        and not (mutex and self.operationMutex.locked())
                        else DISABLED
                    ),
                )
                    ...
                    if b:
                        widgetState(
                            ...
                            (
                                ...
                                if (
                                    enable
                                    and not self.animationsStopped()
                                    and (b != self.stepButton or self.codeText)
                                )
                                else DISABLED
                            ),
                        )
    ...
    def startAnimations(self, enableStops=True, state=None):
        self.animationState = (
            state
            if state in set(Animation)
            else (
                self.animationState
                if self.animationState in (Animation.RUNNING, Animation.STEP)
                else Animation.RUNNING
            )
        )
        ...
        if (
            self.stepButton
            and enableStops
            and (self.codeText or state == Animation.STEP)
        ):
    ...
    def runVisualization(
        self,
    ):  # override of runVisualization that populates default hint
