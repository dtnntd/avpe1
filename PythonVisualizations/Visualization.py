...
class Visualization(object):  # Base class for Python visualizations
    ...
    def __init__(  # Constructor
        ...
        if canvasWidth is None:
            canvasWidth = self.DEFAULT_CANVAS_WIDTH
        if canvasHeight is None:
        ...
        self.canvas = Scrim(
            self.canvasFrame, width=canvasWidth, height=canvasHeight, bg=self.DEFAULT_BG
        )
    ...
    def expandCanvasFor(self, *itemOrBBox):
        ...
        bounds = getattr(self, "canvasBounds", (0, 0, *widgetDimensions(self.canvas)))
        bbox = BBoxUnion(
            *(
                self.canvas.bbox(item) if isinstance(item, int) else item
                for item in itemOrBBox
    ...
    def setCanvasBounds(self, canvasBounds, expandOnly=True):
        if canvasBounds is None:
            ...
            return
        intBounds = (
            math.floor(canvasBounds[0]),
            math.floor(canvasBounds[1]),
            math.ceil(canvasBounds[2]),
            math.ceil(canvasBounds[3]),
        )
        self.canvasBounds = (
            BBoxUnion(self.canvasBounds, intBounds)
            if expandOnly and hasattr(self, "canvasBounds")
            else intBounds
        )
    ...
    def scaleItems(
        ...
        if updateBounds:
            if updateBounds == "simple":
                newBB = [
                    (d - origin) * scaleBy + origin
                    for d, origin in zip(self.canvasBounds, (x0, y0, x0, y0))
                ]
            else:
                newBB = BBoxUnion(
                    *(self.canvas.bbox(i) for i in self.canvas.find_withtag("all"))
                )
    ...
    def setupCanvasZoomHandlers(
        ...
        if any(
            x is None
            for x in (self.canvasBounds, self.canvasHScroll, self.canvasVScroll)
        ):
            return
        ...
        def zoomHandler(event):
            if event.widget != self.canvas:
                print(
                    "Unexpected call to zoomHandler from widget {}".format(event.widget)
                )
            fixPoint = (self.canvas.canvasx(event.x), self.canvas.canvasy(event.y))
            scaleBy = (1 / zoomBy) if event.state & SHIFT else zoomBy
            self.scaleItems(
                x0, y0, scaleBy, scaleBy, fixPoint=fixPoint, updateBounds=updateBounds
            )
        ...
        if items:
            ...
            if edge in (N, S):
                delta = (
                    delta[0],
                    (
                        visible[3] - itemsBBox[1]
                        if edge is S
                        else visible[1] - itemsBBox[3]
                    ),
                )
            elif edge in (W, E):
                delta = (
                    (
                        visible[2] - itemsBBox[0]
                        if edge is E
                        else visible[0] - itemsBBox[2]
                    ),
                    delta[1],
                )
    ...
    def moveItemsBy(  # Animate canvas items moving from their current
        self,
        items,  # location in a direction indicated by a single
        ...
        expand=True,
    ):  # Expand canvas bounds before scrolling if needed
        self.wait(0)
    ...
    def moveItemsBySequence(  # Iterator for moveItemsBy
        ...
        if items and V(delta).len2() >= 0.001:  # If some items and delta
            ...
            for step in range(steps):
                font = changeFont and (
                    endFont[0],
                    (startFont[1] * (steps - (step + 1)) + endFont[1] * (step + 1))
                    // steps,
                )
                for item in items:
                    if item is not None:
    ...
    def moveItemsTo(  # Animate canvas items moving rigidly
        self,
        items,  # to destination locations along line(s)
    ...
    def moveItemsToSequence(  # Iterator for moveItemsTo
        ...
        if items and toPositions:
            steps = max(1, steps)  # Must use at least 1 step
            moveBy = [
                V(V(toPos) - V(fromPos)) / steps
                for toPos, fromPos in zip(
                    toPositions, [self.canvas.coords(item)[:2] for item in items]
                )
            ]
            changeFont = startFont and endFont and startFont != endFont
            ...
            if see and moved:
                self.scrollToSee(
                    moved + (list(see) if isinstance(see, (list, tuple, set)) else []),
                    sleepTime=0,
                    expand=expand,
                )
    ...
    def moveItemsLinearly(  # Animate canvas items moving each of their
        self,
        items,  # coordinates linearly to new destinations
    ...
    def moveItemsLinearlySequence(  # Iterator for moveItemsLinearly
        ...
        if items and toPositions:
            ...
            moveBy = [
                V(V(toPos) - V(fromPos)) / steps
                for toPos, fromPos in zip(
                    toPositions, [self.canvas.coords(item) for item in items]
                )
            ]
            changeFont = startFont and endFont and startFont != endFont
    ...
    def moveItemsOnCurve(  # Animate canvas items moving from their current
        self,
        items,  # location to destinations along a curve
        toPositions,  # items can be a single item or list of items
    ...
    def moveItemsOnCurveSequence(  # Iterator for moveItemsOnCurve
        ...
        if items and toPositions:
            ...
            for step in range(steps):  # Go through all steps of the annimation
                ...
                font = changeFont and (
                    endFont[0],
                    (startFont[1] * toGo + endFont[1] * (step + 1)) // steps,
                )
                ...
                for i, item in enumerate(items):
                    ...
                    if len(coords) == 2:
                        moveBy = V(
                            V(V(toPositions[i]) - V(coords)) / ((toGo + 1) / scale)
                        ).rotate(ang)
                        self.canvas.move(item, *moveBy)
                        if changeFont and self.canvas.type(item) == "text":
                            self.canvas.itemconfigure(item, font=font)
    ...
    def withinCanvas(self, point, visible=False):
        ...
        bounds = (
            self.canvasBounds
            if not visible and self.canvasBounds
            else self.visibleCanvas()
        )
        return (
            bounds[0] <= point[0]
            and point[0] <= bounds[2]
            and bounds[1] <= point[1]
            and point[1] <= bounds[3]
        )
    ...
    def visibleCanvas(self):
        ...
        return [
            ...
            for bounds, dim, pos in zip(
                (Xbounds, Ybounds) * 2,
                canvasDims * 2,
                (max(0, xPos[0]), max(0, yPos[0]), min(1, xPos[1]), min(1, yPos[1])),
            )
        ]
    ...
    def scrollToSee(self, itemsOrBBoxes, sleepTime=0.01, steps=10, **kwargs):
        ...
        xPos = self.canvasHScroll.get()
        yPos = self.canvasVScroll.get()
        if distance2(xPos + yPos, newX + newY) < 0.0001:
            return
        if sleepTime > 0:
    ...
    def scrollSettingsToSee(
        ...
        if debug:
            print(
                (
                    "Request to scroll.  Current visible: {visibleCanvas} "
                    "Bounding box to see: {BBox}"
                ).format(**locals())
            )
        if (
            expand
            and not BBoxEmpty(BBox)
            and (not BBoxContains(self.canvasBounds, BBox))
        ):
            if debug:
                print("Expanding canvas bounds from", self.canvasBounds, end=" ")
        ...
        while keep > 0:
            ...
            if BBoxSize(BBox) <= canvasDims:
                if keep == len(BBoxes) or seen:
                    break
                keep += step
            else:
                keep -= step
            step = max(1, step // 2)
            BBox = BBoxUnion(*(BBoxes[:keep] if firstPriority else BBoxes[-keep:]))
        if debug and 1 < len(BBoxes) and keep < len(BBoxes):
            print(
                (
                    "Reduced requested bounding box to {} keeping {} {} of {} "
                    "boxes so size is {} and canvas is {}"
                ).format(
                    BBox,
                    "first" if firstPriority else "last",
                    keep,
                    len(BBoxes),
                    BBoxSize(BBox),
                    canvasDims,
        ...
        if BBoxSize(BBox) <= canvasDims:
            for pos, XorY in ((xPos, 0), (yPos, 1)):
                lo = max(canvasBounds[XorY], min(visibleCanvas[XorY], BBox[XorY]))
                hi = min(
                    canvasBounds[2 + XorY], max(visibleCanvas[2 + XorY], BBox[2 + XorY])
                )
                if lo < visibleCanvas[XorY] or visibleCanvas[2 + XorY] < hi:
                    pos[0] = (  # Shift lower or higher, but not both
                        (lo if lo < visibleCanvas[XorY] else hi - canvasDims[XorY])
                        - canvasBounds[XorY]
            ...
            if debug:
                if xPos != list(self.canvasHScroll.get()):
                    print(
                        "Changing horizontal scroll from {} to {}".format(
                            self.canvasHScroll.get(), xPos
                        )
                    )
                if yPos != list(self.canvasVScroll.get()):
                    print(
                        "Changing vertical scroll from {} to {}".format(
    ...
    def reconcileItemPositions(self, items, positions):
        ...
        if len(items) != len(positions):
            raise ValueError("Number of items must match length of positions")
...
def categorizeArguments(
    ...
    patterns = (
        nonNegativeNumber,
        signedNumber if signed else negativeNumber,
        option,
        other,
    )
...
if __name__ == "__main__":
    ...
    for name, args in (
        ...
        print(
            "{} argument{}: {}".format(
                name, "" if len(args) == 1 else "s", ", ".join(repr(a) for a in args)
            )
        )
    ...
    app = Visualization(
        title="Visualization test", canvasBounds=(0, 0, 1000, 500) if grid else None
    )

    centerText = app.canvas.create_text(
        app.targetCanvasWidth // 2,
        app.targetCanvasHeight // 2,
        text="Center of the canvas",
        fill=app.VALUE_COLOR,
        font=app.VALUE_FONT,
    )

    def makeLineWithBBox(N=9, color="blue", vertColor="red", bg="old lace", gap=10):
        tags = ("crazyLine", "crazyLineLabel", "crazyBBox")
        for tag in tags:
            app.canvas.delete(tag)
        crazyLineCoords = [
            (
                random.randrange(app.targetCanvasWidth // 2 - gap),
                random.randrange(app.targetCanvasHeight // 2 - gap),
            )
            for j in range(N)
        ]
        bbox = BBoxEnclosing(*flat(*crazyLineCoords))
        crazyBBox = app.canvas.create_rectangle(*bbox, fill=bg, tags="crazyBBox")
        crazyLine = app.canvas.create_line(
            *flat(*crazyLineCoords[:-1]),
            tags="crazyLine",
            arrow=LAST,
            fill=color,
            smooth=True,
        ...
        verts = [
            app.canvas.create_text(*p, text=str(i), fill=vertColor, tags="crazyLine")
            for i, p in enumerate(crazyLineCoords[:-1])
        ]
        crazyLineLabel = app.canvas.create_text(
            *crazyLineCoords[-1], text="Crazy line", fill=color, tags="crazyLineLabel"
        )
        for tag in tags:
            app.canvas.tag_bind(
                tag,
                "<Button>",
                lambda e: makeLineWithBBox(N, color, vertColor, bg, gap),
            )
    ...
    if grid:
        ...
        extremes = [
            app.canvas.create_text(
                8 if dim else end * 10 + 5,
                end * 10 + 5 if dim else 8,
                anchor=E if dim else S,
                text="",
                fill="blue",
                tags="span",
            )
        ...
        for dim in (0, 1):
            ...
            axes.append(
                app.canvas.create_line(
                    ...
                    app.canvasBounds[dim + 2] if dim else mid,
                    fill=color,
                    width=1,
                )
            )
        ...
        LRScrollBtn = app.canvas.create_text(
            canvasSize[0] // 2 + 90, canvasSize[1] // 2 + 50, text="Scroll Right"
        )
        ...
        def toggleScroll(event=None):
            ...
            app.scrollToSee(
                [labels[lastLabel[0] if right else 0]], expand=False, debug=True
            )
