...
class CodeHighlightBlock(object):
    ...
    def __init__(
        self,  # Constructor takes code block, assigns it
        code,  # a unique prefix to highlight snippets in it,
        textWidget,
    ):  # and tags the snippets when refreenced
    ...
    def __getitem__(self, fragment):
        ...
        if span:
            if self.startMark in self.textWidget.mark_names():
                startLine = int(self.textWidget.index(self.startMark).split(".")[0])
                newspan = [
                    "{}.{}".format(startLine + int(line), char)
                ...
                raise KeyError(
                    'Start mark "{}" no longer in CodeHighlightBlock'.format(
                        self.startMark
                    )
                )
        else:
            raise KeyError(
                'Snippet "{}" copy {} not found in code block'.format(*fragment)
            )
    ...
    def findFragment(self, fragment, copy=1):
        ...
        if isinstance(fragment, type(declarationPattern)):
            ...
            while start is not None and copy > 1:
                ...
                copy -= 1
                start = (
                    j + self.code[j:].index(fragment)
                    if fragment in self.code[j:]
                    else None
                )
        if start is None:
            return None
        chars, line = 0, 0
        while line < len(self.lines) and chars + len(self.lines[line]) < start:
        ...
        while line < len(self.lines) and chars + len(self.lines[line]) < start + len(
            fragment
        ):
            chars += len(self.lines[line]) + 1
            line += 1
...
"""
    ...
    fragments = [
        ...
        (re.compile(r"\W(n)\W"), 3),
        re.compile(r"\s(1)\s"),
        "not here",
        re.compile(r"not here"),
    ]
    tags = {}
    ...
    for i, fragment in enumerate(fragments):
        try:
            fragTuple = (
                ...
                else (fragment, 1 + i - fragments.index(fragment))
            )
            copy = fragTuple[1]
            tag = (
                codeHighlightBlock[fragment]
                if copy == 1
                else codeHighlightBlock[fragTuple]
            )
            tags[fragTuple] = tag
            canvasPrint(
                'Tag "{}" relative span is {}'.format(
