...
"""
    ...
    def insertionSort(self, code=insertionSortCode, start=True):
        ...
        while outer < len(self.list):
            ...
            if inner > 0:
                self.highlightCode("temp < self.__a[inner-1]", callEnviron, wait=wait)
            while inner > 0 and temp < self.list[inner - 1].val:
                # Shift cells right that are greater than mark
                self.highlightCode("self.__a[inner] = self.__a[inner-1]", callEnviron)
                ...
                if inner > 0:
                    self.highlightCode(
                        "temp < self.__a[inner-1]", callEnviron, wait=wait
                    )
    ...
    def bubbleSort(self, code=bubbleSortCode, start=True):
        ...
        while last > 0:
            ...
            for inner in range(last):
                ...
                self.highlightCode(
                    "self.__a[inner] > self.__a[inner+1]", callEnviron, wait=wait
                )
    ...
    def selectionSort(self, code=selectionSortCode, start=True):
        ...
        while outer < len(self.list) - 1:
            ...
            if minIndex is None:
                ...
                self.moveItemsBy(
                    minIndex,
                    (self.cellCenter(outer)[0] - self.canvas.coords(minIndex[0])[0], 0),
                    sleepTime=wait / 5,
                )

            # Find the minimum element in remaining unsorted array
            self.highlightCode("inner in range(outer+1, self.__nItems)", callEnviron)
            for inner in range(outer + 1, len(self.list)):
                ...
                self.highlightCode(
                    "self.__a[inner] < self.__a[min]", callEnviron, wait=wait
                )
                if self.list[inner].val < self.list[min_idx].val:
                    ...
                    self.moveItemsBy(
                        ...
                        (
                            ...
                            - self.canvas.coords(minIndex[0])[0],
                            0,
                        ),
                        sleepTime=wait / 5,
                    )
                ...
                self.highlightCode(
                    "inner in range(outer+1, self.__nItems)", callEnviron
                )
                centerX0 = self.cellCenter(inner + 1)[0]
                deltaX = centerX0 - self.canvas.coords(innerIndex[0])[0]
                self.moveItemsBy(innerIndex, (deltaX, 0), sleepTime=wait / 5)
