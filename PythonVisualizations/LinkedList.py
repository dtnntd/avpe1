...
class Node(object):
    ...
    def __init__(
        self, k, nextNode=None, id="", cell=None, value=None, dot=None, nextPointer=None
    ):
        self.key = k
        self.id = id
    ...
    def items(self):  # Return list of canvas items used to draw Node
        return tuple(
            i
            for i in (self.cell, self.value, self.dot, self.nextPointer)
            if i is not None
        )
...
class LinkedList(VisualizationApp):
    ...
    def indexTip(self, pos):  # Compute position of index pointer tip
        ...
        return V(self.x_y_offset(pos)) + (  # Goes to middle top for normal
            V((self.CELL_WIDTH // 2, 0))
            if pos > 0  # position, else
            else V((0, self.CELL_HEIGHT // 2))
        )  # left middle for pos <= 0

    def indexCoords(self, pos, level=0):
        tip = self.indexTip(pos)
        delta = (0, self.CELL_SIZE // 5) if pos >= 0 else (self.CELL_SIZE * 4 // 5, 0)
    ...
    def resizeCanvas(self, event=None):  # Handle canvas resize events
        if self.canvas and self.canvas.winfo_ismapped() and self.animationsStopped():
            width, height = widgetDimensions(self.canvas)
            new_row_length = max(
                2,
                (width - self.LL_X0 + self.CELL_GAP)
                // (self.CELL_WIDTH + self.CELL_GAP),
            )
    ...
    def createDot(self, coordsOrPos, id):
        coords = (
            coordsOrPos
            if isinstance(coordsOrPos, (list, tuple))
    ...
    def nextLinkCoords(self, pos, d=1):
        ...
        p2 = V(tip) - (
            V((0, (1 if dy > 0 else -1) * self.ROW_GAP // 2))
            if stairstep
            else V(delta3)
        )
        return (*p0, *p1, *p2, *tip)
    ...
    def createLink(  # Create  the canvas items for a Link node
        ...
        self.canvas.tag_bind(id, "<Button>", lambda e: self.setArgument(str(val)))
    ...
    def linkedListNode(self):
        ...
        rect = self.canvas.create_rectangle(
            x + self.CELL_WIDTH * 2 // 3,
            y,
            x + self.CELL_WIDTH,
            y + self.CELL_HEIGHT,
            fill=self.UNSORTED_BACKGROUND,
            tags=("LinkedList", "cell"),
        )
        oval = self.createDot(0, "LinkedList")
        ovalCoords = self.canvas.coords(oval)
        firstText = self.canvas.create_text(
            (ovalCoords[0] + ovalCoords[2]) / 2,
            (y + ovalCoords[1]) / 2,
            text="first",
            font=("Courier", -10),
        )
        nameText = self.canvas.create_text(
            x, y + self.CELL_HEIGHT // 2, font=("Courier", -14), text="LinkedList"
        )

    ### ANIMATION METHODS###
    def createIndex(self, pos, name=None, level=0):
        arrow = self.canvas.create_line(
            *self.indexCoords(pos, level), arrow=LAST, fill=self.VARIABLE_COLOR
        )
        if name:
            name = self.canvas.create_text(
                *self.indexLabelCoords(pos, level),
                text=name,
    ...
    def outputData(self, posOrNode=1, callEnviron=None, copy=True, wait=0.1):
        ...
        node = posOrNode if isinstance(posOrNode, Node) else self.list[posOrNode - 1]
        ...
        self.outputBox.setToText(
            toMove, sleepTime=wait / 10, color=self.canvas.itemConfig(toMove[0], "fill")
        )
    ...
    def outputBoxCoords(self, full=False):
        return (
            ...
            self.LL_Y0 // 5,
            self.LL_X0 // 5
            + (self.CELL_WIDTH + self.CELL_GAP) * (self.LEN_ROW if full else 1)
            - self.CELL_GAP,
...
"""
    ...
    def firstData(self, code=firstCode, animateOutput=True, start=True):
        ...
        if self.first is None:
            ...
            self.highlightCode("return self.getFirst().getData()", callEnviron)
            if animateOutput:
...
"""
    ...
    def deleteFirst(self, code=deleteFirstCode, start=True):
        ...
        self.highlightCode("self.setNext(first.getNext())", callEnviron, wait=wait)
        previous = 0
...
"""
    ...
    def newLinkedList(self, code=newLinkedListCode):
        callEnviron = self.createCallEnvironment(code=code, startAnimations=False)
...
"""
    ...
    def insert(self, val, code=insertCode, start=True, wait=0.1):
        ...
        insertLink = self.first if previous == 0 else self.list[previous].nextPointer
        if self.first is None or previous >= len(self.list):
            self.linkNext(previous, 0)
            insertLink = (
                self.first if previous == 0 else self.list[previous].nextPointer
            )
        self.moveItemsLinearly(insertLink, insertLinkCoords, sleepTime=wait / 10)

        toMove = [insertLink] + list(newNode.items())
        for node in self.list[previous:]:
            toMove.extend(node.items())
        toCoords = (
            [self.nextLinkCoords(previous)]
            + [
                self.canvas.coords(item)
                for node in self.list[previous:]
                for item in node.items()
            ]
            + list(self.linkCoords(len(self.list) + 1))
        )
...
"""
    ...
    def delete(self, goal, code=deleteCode, start=True, wait=0.1):
        ...
        self.highlightCode("previous.getNext() is not None:", callEnviron, wait=wait)
        while previous < len(self.list):
            ...
            self.highlightCode("goal == key(link.getData())", callEnviron, wait=wait)
            ...
            if found:
                ...
                self.highlightCode("previous.setNext(link.getNext())", callEnviron)
                updateFirst = previous == 0
                node = self.list[previous]
                nextPointer = node.nextPointer
                toMove = (
                    self.first if updateFirst else self.list[previous - 1].nextPointer,
                    *linkIndex,
                    node.cell,
                    node.value,
                    foundHighlight,
                ...
                self.highlightCode("return link.getData()", callEnviron, wait=wait)
            ...
            self.moveItemsTo(
                previousIndex,
                (self.indexCoords(link, level=1), self.indexLabelCoords(link, level=1)),
                sleepTime=wait / 10,
            )
            self.highlightCode(
    ...
    def restorePositions(  # Move all links on the canvas to their correct
        ...
        addItems=[],
        addCoords=[],  # to some target coordinates
        sleepTime=0.01,
    ):
        if self.first is None:
            return  # No first pointer -> nothing to restore
    ...
    def cleanUp(
        self,  # Customize cleanup to restore link positions
        callEnvironment=None,
        **kwargs
    ):
...
"""
    ...
    def find(self, goal, code=findCode):
        ...
        while link <= len(self.list):
            self.highlightCode("key(link.getData()) == goal", callEnviron, wait=wait)
...
"""
    ...
    def createFoundHighlight(self, pos):  # Highlight the Link cell at pos
        ...
        return self.canvas.create_rectangle(
            *bbox, fill="", outline=self.FOUND_COLOR, width=4, tags="found item"
        )
    ...
    def clickDelete(self):
        ...
        self.setMessage(
            ...
            if empty
            else "{} deleted".format(val) if result else "Item {} not found".format(val)
        )
        if result:
            self.clearArgument()
    ...
    def clickDeleteFirst(self):
        ...
        self.setMessage(
            "Error! Queue is empty" if result is None else "Deleted {}".format(result)
        )
    ...
    def clickGetFirst(self):
        ...
        self.setMessage(
            ...
            if first is None
            else "The first link's data is {}".format(first)
        )
    ...
    def makeButtons(self):
        vcmd = (
            self.window.register(self.validate),
            "%d",
            "%i",
            "%P",
            "%s",
        ...
        newLinkedListButton = self.addOperation(
            "New", lambda: self.clickNewLinkedList(), helpText="Create new, empty list"
        )
        deleteFirstButton = self.addOperation(
            "Delete First",
            lambda: self.clickDeleteFirst(),
            helpText="Delete first item from list",
        )
        getFirstButton = self.addOperation(
            "Get First",
            lambda: self.clickGetFirst(),
            helpText="Get copy of first item from list",
        )
        traverseButton = self.addOperation(
            "Traverse", lambda: self.clickTraverse(), helpText="Traverse items in list"
        )
...
if __name__ == "__main__":
    ...
    try:
        for arg in sys.argv[1:]:
            if len(arg) > ll.maxArgWidth:
                arg = arg[: ll.maxArgWidth]
            ll.insert(arg)
    except UserStop:
        pass
    ll.cleanUp()
