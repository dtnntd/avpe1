...
class GraphBase(VisualizationApp):
    ...
    def __init__(  # Create a graph visualization application
        ...
        **kwargs
        ...
        self.weightValidate = (
            self.window.register(numericValidate),
            "%d",
            "%i",
            "%P",
            "%s",
    ...
    def mapHandler(self):
        def map_handler(event):
            if event.widget == self.window:
                if self.DEBUG:
                    print(
                        "{} event on {}".format(event.type, self.window),
                        "Adjacency matrix panel = {} state = {}".format(
                            self.adjacencyMatrixPanel, self.adjacencyMatrixPanel.state()
                        ),
                    )
    ...
    def createAdjacencyMatrixPanel(self, suffix=" Adjacency Matrix", anchor=SE):
        ...
        self.matrixExpose = Button(
            self.adjMatControlBar,
            image=self.adjMatControlImages["collapse"],
        ...
        self.buttonPadX = int(self.window.winfo_fpixels(self.matrixExpose["padx"]))
        ...
        if not (
            isinstance(self.window, (Tk, Toplevel)) and sys.platform.startswith("win")
        ):
        ...
        if hasattr(self.window, "title"):  # If top window is exposed
            ...
            if self.DEBUG:
                print(
                    "Withdrawing adjacency matrix until {} exposed".format(self.window)
                )
            self.adjacencyMatrixPanel.withdraw()
    ...
    def createAdjacencyMatrixControlImages(self, height=None):
        ...
        self.adjMatControlImages = dict(
            (name, getPhotoImage(name + "-symbol.png", targetSize)) for name in names
        )
    ...
    def toggleAdjacencyMatrixDisplay(self):
        if self.adjMatrixFrame in self.adjacencyMatrixPanel.pack_slaves():
            buttonImage(self.matrixExpose, self.adjMatControlImages["uncollapse"])
    ...
    def positionAdjacencyMatrix(self, anchor=None):
        if anchor is None:
            anchor = self.adjacencyMatrixAnchor
        self.adjacencyMatrixAnchor = anchor
        self.window.update_idletasks()
        if self.DEBUG:
            print(
                "Repositioning adjacency matrix",
                "panel {} state = {}".format(
                    self.adjacencyMatrixPanel, self.adjacencyMatrixPanel.state()
                ),
                "App window {} state = {}".format(self.window, self.window.state()),
            )
        if self.window.state() == NORMAL:
            if not (
                hasattr(self.adjacencyMatrixPanel, "trans_parent")
                and self.adjacencyMatrixPanel.winfo_geometry().endswith("+0+0")
            ):
                if self.DEBUG:
                    print(
                        "Setting trans_parent of adj mat panel",
                        self.adjacencyMatrixPanel,
                        "with geometry",
                        self.adjacencyMatrixPanel.winfo_geometry(),
                    )
                self.adjacencyMatrixPanel.transient(self.controlPanel)
                setattr(self.adjacencyMatrixPanel, "trans_parent", self.controlPanel)
            if (
                sys.platform.startswith("win")
                or self.adjacencyMatrixPanel.state() != NORMAL
            ):
                self.adjacencyMatrixPanel.deiconify()
        shown = (
            self.adjMatrixFrame
        ...
        x = x0 + max(
            0, (0 if W in anchor else canW - w if E in anchor else int(canW - w) // 2)
        )
        y = y0 + max(
            0, (0 if N in anchor else canH - h if S in anchor else int(canH - h) // 2)
        )
    ...
    def newGraph(self):
        ...
        if self.DEBUG:
            for event in (
                ...
                self.window.bind("<{}>".format(event), genericEventHandler(), "+")
    ...
    def display(self):
        ...
        self.canvas.tag_bind(
            self.graphRegionRectangle, "<Double-Button-1>", self.newVertexHandler()
        )
        self.vertexTable = Table(
            self,
            (self.graphRegion[2] + 50, 40),
            label="_vertices",
            vertical=True,
            cellHeight=self.VERTEX_RADIUS,
            labelAnchor=SE,
    ...
    def createVertexItems(self, label, coords=None, color=None, tags=("vertex",)):
        ...
        shape = self.canvas.create_oval(
            *(V(coords) - self.vRadius),
            *(V(coords) + self.vRadius),
            tags=tags + ("shape", label),
            outline="",
            width=1,
            fill=color,
        ...
        for item in items:
            for btn in (1, 2, 3):
                self.canvas.tag_bind(item, "<Button-{}>".format(btn), startMoveHandler)
                self.canvas.tag_bind(item, "<B{}-Motion>".format(btn), moveHandler)
                self.canvas.tag_bind(
    ...
    def createEdgeItems(
        ...
        line = self.canvas.create_line(
            ...
            *p2,
            ...
            tags=tags + ("line",),
            activewidth=self.ACTIVE_EDGE_WIDTH,
            splinesteps=steps,
            smooth=True
        )
        weightText = (
            self.canvas.create_window(
                *weightCenter,
                state=HIDDEN if weight == 0 else NORMAL,
                tags=tags + ("weightWindow",),
                window=self.createEdgeWeightEntry(
                    self.GRAPH_REGION_BACKGROUND, edgePair, weight, self.canvas
                )
            )
            if self.weighted
            else self.canvas.create_text(
                *weightCenter,
        ...
        if edgePair:
            self.canvas.tag_bind(
                line, "<Double-Button-1>", self.deleteEdgeHandler(edgePair)
            )
    ...
    def edgeCoords(self, base, tip, removeRadius=VERTEX_RADIUS):
        ...
        inflection = (
            midPoint if self.bidirectionalEdges.get() else V(midPoint) + V(offset)
        )
    ...
    def createLabeledArrow(
        ...
        **kwargs
        ...
        coords = (
            self.labeledArrowCoords(vertexOrCoords, **kwargs)
            if isinstance(vertexOrCoords, (int, str))
            or (
                isinstance(vertexOrCoords, (list, tuple))
                and len(vertexOrCoords) == 2
                and isinstance(vertexOrCoords[0], (int, float))
            )
            else vertexOrCoords
        )
        if anchor is None:
    ...
    def labeledArrowAnchor(self, arrowCoords):
        ...
        return (W if dx < 0 else E) if abs(dx) > abs(dy) else (N if dy < 0 else S)

    def labeledArrowCoords(
        self, vertexOrCoords, level=1, orientation=0, offset=5, **kwargs
    ):
        center = (
            self.vertexCoords(vertexOrCoords)
            if isinstance(vertexOrCoords, str)
            else (
                self.vertexCoords(self.vertexTable[vertexOrCoords].val)
                if isinstance(vertexOrCoords, int)
                and vertexOrCoords < len(self.vertexTable)
                else vertexOrCoords
            )
        )
        Vrad = V(V(0, self.VERTEX_RADIUS + offset).rotate(orientation))
        tip = V(center) - Vrad
        Vdelta = V(V(0, abs(level) * abs(self.VARIABLE_FONT[1])).rotate(orientation))
        base = V(tip) - Vdelta
    ...
    def startMoveHandler(self, vertexLabel):
        def startHandler(event):
            if self.DEBUG:
                print(
                    'Entering startHandler {}B{} on "{}" #{}'.format(
                        "Shift-" if event.state & SHIFT else "",
                        event.num,
                        vertexLabel,
                        event.serial,
                    )
            ...
            if self.dragItems:
                if self.DEBUG:
                    print(
                        'Exiting startHandler #{} dragItems = {}"'.format(
            ...
            selectionIndex = (
                1
                if (
                    len(self.selectedVertices) > 0
                    and (
                        event.state & SHIFT
                        or isinstance(event.num, int)
                        and event.num != 1
                    )
                )
                else 0
            ...
            if self.DEBUG:
                print(
                    'Finished startHandler on "{}" #{}'.format(
    ...
    def moveHandler(self, vertexLabel):
        def mvHandler(event):
            moveVertex = event.state & (SHIFT | MOUSE_BUTTON_2 | MOUSE_BUTTON_3) or (
            ...
            if self.DEBUG:
                ...
                print(
                    'Entering mvHandler B{} on {} "{}" #{}'.format(
                        btn,
                        "vertex" if moveVertex else "new edge for",
                        vertexLabel,
                        event.serial,
                    )
                )
            if not self.operationMutex.acquire(blocking=False):
                self.setMessage(
                    "Cannot {} during other operation".format(
            ...
            if dragItemType == "oval":
                ...
                if not bad:
                    for orig, copy in zip(
                        self.vertices[vertexLabel].items, self.dragItems[:2]
                    ):
                ...
                p0, p1, p2, steps, wc = self.edgeCoords(
                    self.vertexCoords(vertexLabel), self.lastPos, removeRadius=0
                )
                ...
                raise Exception(
                    "Dragging items of type {} not supported".format(dragItemType)
                )
            self.operationMutex.release()
            if self.DEBUG:
                print(
                    'Finished mvHandler on "{}" #{}'.format(vertexLabel, event.serial)
                )
        ...
        for tag in tags:
            for item in self.canvas.find_withtag(tag):
                if self.canvas.type(item) == "oval":
                    ...
                    self.canvas.itemconfigure(item, fill=self.VALUE_COLOR, state=NORMAL)

    def releaseHandler(self, vertexLabel):
        def relHandler(event):
            if self.DEBUG:
                print(
                    'Entering relHandler B{} on "{}" #{}'.format(
                ...
                if dragItemType == "oval":
                    ...
                    if bad:
                        if self.DEBUG:
                            print(
                                (
                                    "Release position is bad, "
                                    "ending #{}. Calling undoDrag"
                                ).format(event.serial)
                            )
                        ...
                        if self.DEBUG:
                            print(
                                'Finished release of vertex "{}"  #{}.'.format(
                                    vertexLabel, event.serial
                                )
                            )
                elif dragItemType == "line":
                    toVert = self.findVertex(
                        (event.x, event.y), exclude=self.vertexPlusAdjacent(vertexLabel)
                    )
                    self.restoreVertexAppearance()
                    if toVert:
                        if self.DEBUG:
                            print(
                                'Release new edge to "{}"  #{}.'.format(
                                    toVert, event.serial
                                )
                            )
                        self.dispose({}, *self.dragItems)
                        ...
                        if self.DEBUG:
                            print(
                                (
                                    'Released edge connects "{}" to nothing, '
                                    "ending #{}. Calling undoDrag"
    ...
    def undoDrag(self, vertexLabel, after=None, endDistance=16, serial=0):
        ...
        if self.DEBUG:
            print(
                'Entered undoDrag #{} moving {} for vertex "{}" & after={}'.format(
                    serial, dragItemType, vertexLabel, after
                ),
                end=" ",
            )
        if not (dragItemType in ("line", "oval") and vertexLabel in self.vertices):
            if self.dragItems:
                self.dispose({}, *self.dragItems)
            self.dragItems = None
            if self.DEBUG:
                print(
                    'Unexpected end to undoing {} drag #{} for vertex "{}".'.format(
                        dragItemType, serial, vertexLabel
                    )
                )
            return
        wait = 30
        if dragItemType == "oval":
            ...
            if delta.len2() <= endDistance:
                ...
                self.dragItems = None
                if self.DEBUG:
                    print("Finished undoing {} drag #{}.".format(dragItemType, serial))
                if callable(after):
                    if self.DEBUG:
                        print("Calling after...")
                    after()
                ...
                self.undoDragRequest = self.canvas.after(
                    wait, lambda: self.undoDrag(vertexLabel, after, endDistance, serial)
                )
        elif dragItemType == "line":
            ...
            if distance2(base, newTip) <= endDistance:
                ...
                p0, p1, p2, steps, wc = self.edgeCoords(base, newTip, removeRadius=0)
                self.canvas.coords(self.dragItems[0], *p0, *p1, *p2)
                self.canvas.itemconfigure(self.dragItems[0], splinesteps=steps)
                self.canvas.coords(self.dragItems[1], *wc)
                if self.DEBUG:
                    print(
                        "Scheduling more undoing of {} drag #{} after {} ms.".format(
                            dragItemType, serial, wait
                        )
                    )
                self.undoDragRequest = self.canvas.after(
                    wait, lambda: self.undoDrag(vertexLabel, after, endDistance, serial)
    ...
    def selectVertex(self, vertexLabel, vID=0, tags="selected", checkMutex=True):
        ...
        if selection:
            if vertexLabel is None:
                ...
                self.canvas.coords(
                    selection[1], *(Vcenter - Vradius), *(Vcenter + Vradius)
                )
            ...
            self.selectedVertices[vID] = (
                ...
                self.canvas.create_oval(
                    ...
                    *(Vcenter + Vradius),
                    fill="",
                    outline=self.SELECTED_OUTLINES[vID],
    ...
    def deleteVertexHandler(self, vertexLabel):
        def delVertHandler(event):
            if self.DEBUG:
                print(
                    'Entered delVertHandler for "{}" #{}.'.format(
                        vertexLabel, event.serial
                    )
                )
            if not (vertexLabel in self.vertices and self.dragItems):
                if self.DEBUG:
                    print(
                        (
                            'Exit delVertHandler #{} on "{}" with ' "dragItems = {}"
            ...
            if self.DEBUG:
                print(
                    'Finished delVertHandler for "{}" #{}.'.format(
                        vertexLabel, event.serial
                    )
                )
    ...
    def deleteEdgeHandler(self, edge):
        def delEdgeHandler(event):
            if self.DEBUG:
                print("Entered delEdgeHandler for {} #{}.".format(edge, event.serial))
            if not (edge in self.edges):
                if self.DEBUG:
                    print("Exit delEdgeHandler #{} on {}".format(event.serial, edge))
                return
            if self.deleteEdge(*edge):
                self.setMessage("Deleted edge {}".format(edge))
            self.enableButtons()
            if self.DEBUG:
                print("Finished delEdgeHandler for {} #{}.".format(edge, event.serial))
    ...
    def findVertex(self, point, exclude=()):
        for v in self.vertices:
            if v not in exclude:
                if distance2(point, self.vertexCoords(v)) <= self.VERTEX_RADIUS**2:
                    return v
    ...
    def newVertexHandler(self):
        def newVertHandler(event):
            secondary = event.state & (SHIFT | MOUSE_BUTTON_2 | MOUSE_BUTTON_3) or (
                isinstance(event.num, int) and event.num != 1
            )
            ...
            if secondary:
                ...
                self.setMessage(
                    "Maximum number of vertices = {}".format(self.MAX_VERTICES)
                )
            self.enableButtons()
    ...
    def badPosition(self, center, border=None, ignore=()):
        ...
        """
        if border is None:
            border = self.VERTEX_RADIUS
        return (
            not BBoxContains(V(self.graphRegion) + V(V(1, 1, -1, -1) * border), center)
            or any(
                distance2(center, self.canvas.coords(v.items[1]))
                < (2 * self.SELECTED_RADIUS) ** 2
                for v in self.vertices.values()
                if v.val[0] not in ignore
            )
            or (
                self.bidirectionalEdges.get()
                and any(
                    collinearBetween(
                        self.vertexCoords(self.vertexTable[j].val),
                        center,
                    ...
                    if self.edgeWeight(self.vertexTable[j].val, self.vertexTable[k].val)
                    > 0
                )
            )
        )
    ...
    def createVertex(self, label, color=None, coords=None, tags=("vertex",)):
        if self.nVertices() >= self.MAX_VERTICES:
            self.setMessage(
                "Already have max Number of vertices {}".format(self.MAX_VERTICES)
            )
            return
        if not self.operationMutex.acquire(blocking=False):
        ...
        try:
            self.cleanUp()
            self.setMessage("")
            items = self.createVertexItems(label, color=color, coords=coords, tags=tags)
        ...
        for item in self.vertexTable[-1].items:
            self.canvas.tag_bind(item, "<Button-1>", lambda e: self.setArgument(label))
        ...
        for otherVert in self.vertices.values():
            if vert == otherVert:
                ...
                frame.grid(row=vertID, column=vertID, sticky=(N, E, S, W))
            else:
                otherVertColor = self.canvas_itemConfig(otherVert.items[0], "fill")
                entry = self.createEdgeWeightEntry(
                    vertColor, (vert.val[0], otherVert.val[0]), None
                )
                entry.grid(row=vertID, column=otherVert.val[1], sticky=(N, E, S, W))
                self.adjMat[vert.val[0], otherVert.val[0]] = [0, entry]
                entry = self.createEdgeWeightEntry(
                    otherVertColor, (otherVert.val[0], vert.val[0]), None
                )
                entry.grid(row=otherVert.val[1], column=vertID, sticky=(N, E, S, W))
                self.adjMat[otherVert.val[0], vert.val[0]] = [0, entry]
                maxWidth = max(
                    maxWidth, textWidth(self.ADJACENCY_MATRIX_FONT, otherVert.val[0])
                )
        ...
        for ID in allColumns:
            self.adjMatrixFrame.columnconfigure(
                ID, minsize=maxWidth + self.buttonPadX if ID in columnIDs else 0
            )
        self.positionAdjacencyMatrix()
        self.operationMutex.release()
        return True
    ...
    def deleteVertex(self, vertexLabel):
        if not vertexLabel in self.vertices:
            raise ValueError("Cannot update non-existant vertex {}".format(vertexLabel))
        if not self.operationMutex.acquire(blocking=False):
        ...
        if self.weighted:
            ...
            def edgeWeightChange(event):
                ...
                if not (
                    ...
                    or event.keysym in ("Delete", "BackSpace")
                ):
                    if self.DEBUG:
                        print(
                            "Ignoring char",
            ...
            entry.bind(
                "<FocusIn>", lambda event: event.widget.select_range(0, END), "+"
            )
            entry.bind(
                "<FocusOut>",
                lambda event: self.weight(event.widget, self.edgeWeight(*edge))
                or event.widget.configure(bg=color),
                "+",
            )
        else:
            entry = Button(
    ...
    def weight(self, weightEntry, newWeight=None):
        ...
        if isinstance(weightEntry, str):
            try:
                ...
                raise ValueError(
                    'String "{}" is not a recognized widget'.format(weightEntry)
                )
        if isinstance(weightEntry, Entry):
            ...
            raise ValueError(
                "Cannot set weight in widget of type {}".format(type(weightEntry))
            )
    ...
    def findEdges(self, vertex, inbound=True, outbound=True, unique=None):
        ...
        """
        if unique is None:
            unique = self.bidirectionalEdges.get()
        return [
            edge
            for edge in self.edges
            if (inbound and edge[1] == vertex or outbound and edge[0] == vertex)
    ...
    def edgeWeight(self, fromVert, toVert, weight=None):
        ...
        edges = (
            (edge, revEdge)
            if (self.bidirectionalEdges.get() and weight is not None)
            else (edge,)
        )
        for ed in edges:
            if ed not in self.adjMat and (weight is None or weight == 0):
                raise Exception("Missing adjacency matrix entry {}->{}".format(*ed))
        ...
        if weight > 0:  # For new/updated edges, update canvas display
            if edge not in self.edges:
                ...
                self.weight(
                    self.canvas.itemConfig(self.edges[edge].items[1], "window"), weight
                )
    ...
    def createEdge(self, fromVert, toVert, weight=1, tags=("edge",), checkMutex=True):
        edgeKey = (fromVert, toVert)
        if (
            fromVert == toVert
            or edgeKey in self.edges
            and isinstance(self.edge[edgeKey], drawnValue)
        ):
            return
        ...
        self.edges[edgeKey] = drawnValue(
            edgeKey,
            *self.createEdgeItems(
                centers[0], centers[1], weight=weight, tags=tags, edgePair=edgeKey
            )
        )
        if self.bidirectionalEdges.get():
    ...
    def makeButtons(self, bidirectional=None):
        ...
        self.newGraphButton = self.addOperation(
            "New Graph", self.clickNewGraph, helpText="Create new, empty graph"
        )
        self.bidirectionalEdges = IntVar()
        self.bidirectionalEdges.set(1 if bidirectional is None or bidirectional else 0)
        self.bidirectionalEdgesButton = (
            self.addOperation(
                "Bidirectional",
                self.clickBidirectionalEdges,
                buttonType=Checkbutton,
                variable=self.bidirectionalEdges,
    ...
    def enableButtons(self, enable=True):
        ...
        widgetState(
            ...
            (
                NORMAL
                if enable and self.nVertices() > 0 and self.getArgument(0)
                else DISABLED
            ),
        )
    ...
    def randomFill(self, nVertices):
        maxLabel = (
            self.DEFAULT_VERTEX_LABEL
            if len(self.vertices) == 0
            else self.nextVertexLabel(max(v for v in self.vertices))
        )
    ...
    def clickRandomFill(self):
        val = self.validArgument()
        if val and val.isdigit() and int(val) <= self.MAX_VERTICES - self.nVertices():
            ...
            self.setMessage(
                "Number of vertices must be {} or less".format(
    ...
    def process_command_line_arguments(self, argv):
        ...
        for arg in negative:
            ...
            self.setArgument(chr(ord(self.DEFAULT_VERTEX_LABEL) + self.nVertices()))
        for arg in otherArgs:
            edgeMatch = edgePattern.fullmatch(arg)
            if edgeMatch and all(edgeMatch.group(i) in otherArgs for i in (1, 2)):
                edges.append(
                    (
                        edgeMatch.group(1),
                        edgeMatch.group(2),
...
if __name__ == "__main__":
    argv = sys.argv[1:]
    graph = GraphBase(
        weighted=any(":" in arg for arg in argv)
        or set(argv) & set(("-TuralaMST", "-TuralaSP"))
    )
