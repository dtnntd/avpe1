...
"""
    ...
    def adjacentVertices(self, n, code=adjacentVerticesCode, wait=0.1):
        ...
        if code:
            ...
            nVertArrow = self.createLabeledArrow(nLabel, "n", see=True, **nVertConfig)
        ...
        for j in self.vertexIndices():
            ...
            if code:
                ...
                if j != n:
                    ...
                    edgeEntry.grid(
                        padx=self.ADJ_MAT_HIGHLIGHT_PAD,  # mat edge
...
"""
    ...
    def adjacentUnvisitedVertices(
        self, n, visited, markVisits=True, code=adjacentUnvisitedVerticesCode, wait=0.1
    ):
        nLabel = self.vertexTable[n].val if n < len(self.vertexTable) else None
        nVal = "{} ('{}')".format(n, nLabel)
        callEnviron = self.createCallEnvironment(code=code.format(**locals()))
        ...
        if code:
            ...
            self.highlightCode("j in self.adjacentVertices(n)", callEnviron, wait=wait)
        ...
        for j in self.adjacentVertices(
            ...
            if code:
                ...
                if jArrow is None:
                    ...
                    self.moveItemsTo(
                        jArrow + jVertArrow,
                        self.vertexTable.labeledArrowCoords(j, **jArrowConfig)
                        + self.labeledArrowCoords(
                            self.vertexTable[j].val, **jVertConfig
                        ),
                        sleepTime=wait / 10,
                        see=True,
                        expand=True,
                ...
                if not visited[j].val:
                    self.highlightCode(("markVisits", 2), callEnviron, wait=wait)

            if not visited[j].val:
                if markVisits:
                    if code:
                        self.highlightCode("visited[j] = True", callEnviron, wait=wait)
                    visited[j].val = True
                    if code:
            ...
            if code:
                self.highlightCode(
                    "j in self.adjacentVertices(n)", callEnviron, wait=wait
                )
                colors = self.canvas.fadeItems(localVars)
...
"""
    ...
    def depthFirst(self, n, code=depthFirstCode, wait=0.1):
        nLabel = (
            n
            if isinstance(n, str)
            else self.vertexTable[n].val if n < len(self.vertexTable) else None
        )
        ...
        nArrowConfig = {"level": 1, "anchor": SE}
        nArrow = self.vertexTable.createLabeledArrow(n, "n", see=True, **nArrowConfig)
        ...
        if code:
            ...
            copyItems = tuple(
                self.canvas.copyItem(item) for item in self.vertices[nLabel].items
            )
        ...
        while len(stack) > 0:
            ...
            if code:
                self.highlightCode("visit = stack.peek()", callEnviron, wait=wait)
                if visitArrow is None:
                    ...
                    self.moveItemsTo(
                        ...
                        stack.labeledArrowCoords(len(stack) - 1, **visitArrowConfig)
            ...
            if code:
                ...
                if adjArrow is None:
                    ...
                    self.moveItemsTo(
                        adjArrow,
                        self.labeledArrowCoords(lowerRight, **adjArrowConfig),
                        sleepTime=wait / 10,
                        see=True,
                        expand=True,
                    )
                self.highlightCode(self.innerLoopIterator, callEnviron, wait=wait)
                colors = self.canvas.fadeItems(localVars)
            ...
            for j in self.adjacentUnvisitedVertices(
                ...
                if code:
                    ...
                    self.moveItemsTo(
                        adjArrow,
                        self.labeledArrowCoords(adjVertex, **adjArrowConfig),
                        sleepTime=wait / 10,
                        see=True,
                        expand=True,
                    )
            ...
            if code:
                ...
                if adj is not None:
                    self.highlightCode("stack.push(adj)", callEnviron, wait=wait)
            if adj is not None:
                stack.append(drawnValue(adjVertex))
                if code:
                    copyItems = tuple(
                        self.canvas.copyItem(i) for i in self.vertices[adjVertex].items
                    )
    ...
    def createVisitedTable(
        ...
        visited = Table(
            self,
            (
                self.vertexTable.x0 + self.vertexTable.cellWidth + gap,
                self.vertexTable.y0,
            ),
                ...
                dValue.items = (
                    self.canvas.create_rectangle(
                        *visited.cellCoords(j),
                        fill=self.VISITED_COLORS[initialValue],
                        width=0,
                        outline=self.VISITED_HIGHLIGHT_COLOR,
                        tags=tags
                    ),
    ...
    def updateVisitedCells(self, visited, index=None):
        for v in visited[
            0 if index is None else index : len(visited) if index is None else index + 1
        ]:
            self.canvas.itemconfigure(v.items[0], fill=self.VISITED_COLORS[v.val])
            self.canvas.itemconfigure(v.items[1], text=self.VISITED_SYMBOLS[v.val])
...
"""
    ...
    def breadthFirst(self, n, code=breadthFirstCode, wait=0.1):
        ...
        while len(queue) > 0:
            ...
            visit = self.getVertexIndex(visitLabel)
            if code:
                self.highlightCode("visit = queue.remove()", callEnviron, wait=wait)
                if visitArrow is None:
                    ...
                    self.moveItemsTo(
                        visitArrow,
                        queue.labeledArrowCoords(len(queue) - 1, **visitArrowConfig),
                        sleepTime=wait / 10,
                        see=True,
                        expand=True,
                    )
            ...
            if code:
                for item in visitDValue.items:
                    self.canvas.tag_lower(item, self.vertices[visitLabel].items[0])
                self.moveItemsLinearly(
                    visitArrow + visitDValue.items + flat(*(dv.items for dv in queue)),
                    self.labeledArrowCoords(visitLabel, **visitArrowConfig)
                    + tuple(
                        self.canvas_coords(item)
                        for item in self.vertices[visitLabel].items
                    )
                    + flat(
                        *(
                            (queue.cellCoords(j), queue.cellCenter(j))
            ...
            for j in self.adjacentUnvisitedVertices(
                ...
                if code:
                    ...
                    self.highlightCode("queue.insert(j)", callEnviron, wait=wait)
            ...
            if code:
                self.canvas.restoreItems(localVars, colors)
                self.highlightCode("not queue.isEmpty()", callEnviron, wait=wait)
...
"""
    ...
    def traverseExample(
        self, order, startVertex, code=traverseExampleCode, start=True, wait=0.1
    ):
        orderings = ("breadth", "depth")
        if order not in orderings:
            raise ValueError("Traverse order must be in {}".format(orderings))
        startVal = "{} ('{}')".format(self.getVertexIndex(startVertex), startVertex)
        ...
        iterator = "vertex{vars} in graph.{order}First(start={startVal})".format(
            **locals()
        )
        ...
        for thing in (self.depthFirst if order == "depth" else self.breadthFirst)(
            ...
            if vertexArrow is None:
                ...
                self.moveItemsTo(
                    ...
                    self.vertexTable.labeledArrowCoords(vertex, **vertexArrowConfig)
            ...
            for edge in edgesInPath:
                self.canvas.itemconfigure(
                    edge, width=self.ACTIVE_EDGE_WIDTH, fill=self.ACTIVE_EDGE_COLOR
                )
            self.highlightCode("print(graph.getVertex(vertex))", callEnviron, wait=wait)
...
"""
    ...
    def minimumSpanningTree(
        ...
        self.highlightCode("vMap = [None] * self.nVertices()", callEnviron, wait=wait)
        vMap = Table(
            self,
            (self.vertexTable.x0 + self.vertexTable.cellWidth + 5, self.vertexTable.y0),
            *([None] * self.nVertices()),
            label="vMap",
            labelAnchor=S,
            vertical=True,
            ...
            indicesFont=self.vertexTable.labelFont,
            indicesAnchor=W,
            indicesOffset=30
        )
        callEnviron |= set(vMap.items())
        localVars += vMap.items()
        faded += (Scrim.FADED_FILL, *((Scrim.FADED_OUTLINE,) * (len(vMap.items()) - 1)))

        vertexArrow, vertexArrowConfig = None, {}
        vertexVertArrow = None
        vertexVertConfig = {"orientation": 30, "anchor": SW}
        self.highlightCode("vertex, path in self.depthFirst(n)", callEnviron, wait=wait)
        ...
        for vertex, path in self.depthFirst(n):
            ...
            self.highlightCode(
                "vMap[vertex] = tree.nVertices()", callEnviron, wait=wait
            )
            ...
            self.highlightCode(
                "tree.addVertex(self.getVertex(vertex))", callEnviron, wait=wait
            )
            ...
            if len(treeVerts) == 0:
                ...
                tipCoords = self.labeledArrowCoords(
                    vertexLabel,
                    orientation=V(V(inflection) - V(vertCoords)).orient2d() + 90,
                )[0][2:]
                ...
                localVars, faded = (*localVars, treeArrow), (*faded, Scrim.FADED_FILL)
            ...
            treeVertHighlight = self.canvas.create_oval(
                ...
                *(V(vertCoords) + vRad),
                fill="",
                outline=self.HIGHLIGHTED_VERTEX_COLOR,
                width=self.HIGHLIGHTED_VERTEX_WIDTH,
                tags=MSTtags
            )
            self.canvas.tag_lower(
                treeVertHighlight, self.vertices[vertexLabel].items[0]
            )
            copies = tuple(
                self.canvas.copyItem(item) for item in self.vertexTable[vertex].items
            )
            newItems = (treeVertHighlight, *copies)
            callEnviron |= set(newItems)
            localVars, faded = localVars + newItems, faded + (Scrim.FADED_FILL,) * len(
                newItems
            )
            self.moveItemsLinearly(
                copies,
                (
                    treeVerts.cellCoords(len(treeVerts)),
    ...
    def createVMapArrow(self, vMap, index, vertex, see=()):
        ...
        dot = self.canvas.create_oval(
            *(V(base) - VdotRadius), *(V(base) + VdotRadius), fill="red", width=0
        )
        if see:
            self.scrollToSee(
                (arrow, dot)
...
"""
    vertsByDegreeConstructor = re.compile(r"vertsByDegree = .*\n.*\n.*1\)\)\]")

    def topologicalSort(self, code=topologicalSortCode, start=True, wait=0.1):
        sortTags = "TSort"
        callEnviron = self.createCallEnvironment(code=code, startAnimations=start)
        localVars, faded = (), ()
        ...
        self.highlightCode(self.vertsByDegreeConstructor, callEnviron, wait=wait)
        self.dispose(callEnviron, edgeCountLabel)
        vertsByDegreeLabel = self.canvas.create_text(
            *(V(vertsByDegreeCoords) + V(-15, 15)),
            anchor=E,
            text="vertsByDegree",
            font=self.vertexTable.labelFont,
            fill=self.VARIABLE_COLOR,
            tags=sortTags
        )
        callEnviron.add(edgeCountLabel)
        ...
        callEnviron |= set(
            (vertsByDegreeLabel, *flat(*(tbl.items() for tbl in vertsByDegree)))
        )

        self.highlightCode("inDegree = [0] * self.nVertices()", callEnviron, wait=wait)
        gap = 5
        inDegree = Table(
            self,
            (
        ...
        for vertex in self.vertexIndices():
            ...
            self.highlightCode(
                "inDegree[vertex] = self.degree(vertex)[0]", callEnviron, wait=wait
            )
            colors = self.canvas.fadeItems(localVars, faded)
            ...
            self.moveItemsTo(
                ...
                sleepTime=wait / 10,
                startFont=self.canvas.getItemFont(inb),
                endFont=self.ADJACENCY_MATRIX_FONT,
            )
            ...
            self.highlightCode(
                "vertsByDegree[inDegree[vertex]][vertex] = 1", callEnviron, wait=wait
            )
            copies = [
                self.canvas.copyItem(item) for item in self.vertexTable[vertex].items
            ]
            ...
            self.moveItemsTo(
                copies,
                (VbyDtable.cellCoords(nVerts), VbyDtable.cellCenter(nVerts)),
                sleepTime=wait / 10,
                see=True,
                expand=True,
            )
        ...
        while len(vertsByDegree[0]) > 0:
            self.highlightCode(
                "vertex, _ = vertsByDegree[0].popitem()", callEnviron, wait=wait
            )
            dValue = vertsByDegree[0].pop()
            vertexLabel = dValue.val
            self.dispose(callEnviron, *vertexArrow, dValue.items[0])
            self.canvas.tag_lower(dValue.items[1], self.vertices[vertexLabel].items[0])
            self.moveItemsTo(
            ...
            self.highlightCode(
                "s in self.adjacentVertices(vertex)", callEnviron, wait=wait
            )
            colors = self.canvas.fadeItems(localVars, faded)
            for s in self.adjacentVertices(self.getVertexIndex(vertexLabel)):
                ...
                self.highlightCode(
                    "vertsByDegree[inDegree[s]]", callEnviron, wait=wait / 10
                )
                ...
                if indegreeArrow is None:
                    ...
                    self.moveItemsTo(
                        indegreeArrow,
                        VbyDtable.labeledArrowCoords(-0.5, **indegreeArrowConfig),
                        sleepTime=wait / 10,
                        see=True,
                        expand=True,
                    )
                ...
                self.highlightCode(
                    "vertsByDegree[inDegree[s]].pop(s)", callEnviron, wait=wait
                )
                itemIndex = [v.val for v in VbyDtable].index(sVertexLabel)
                ...
                self.moveItemsOnCurve(
                    popValue.items
                    + flat(
                        *(VbyDtable[j].items for j in range(itemIndex, len(VbyDtable)))
                    ),
                    (
                    ...
                    + flat(
                        *(
                            (VbyDtable.cellCoords(j), VbyDtable.cellCenter(j))
                ...
                inDegree[s].val -= 1
                self.canvas.itemConfig(inDegree[s].items[0], text=str(inDegree[s].val))
                VbyDtable = vertsByDegree[inDegree[s].val]
                self.moveItemsTo(
                    indegreeArrow,
                    VbyDtable.labeledArrowCoords(-0.5, **indegreeArrowConfig),
                ...
                self.highlightCode(
                    "vertsByDegree[inDegree[s]][s] = 1", callEnviron, wait=wait
                )
                self.moveItemsTo(
                    popValue.items,
                    (
        ...
        self.highlightCode("len(result) == self.nVertices()", callEnviron, wait=wait)
        if len(result) == self.nVertices():
...
"""
    ...
    def degree(self, n, code=degreeCode, wait=0.1):
        ...
        for j in self.vertexIndices():
            ...
            if j != n:
                ...
                edgeEntry.grid(
                    padx=self.ADJ_MAT_HIGHLIGHT_PAD,  # adj matrix
                    pady=self.ADJ_MAT_HIGHLIGHT_PAD,
                )
    ...
    def enableButtons(self, enable=True):
        ...
        for btn in (
            self.depthFirstTraverseButton,
            self.breadthFirstTraverseButton,
            self.MSTButton,
        ):
            widgetState(  # can only traverse when start node selected
                btn, NORMAL if enable and self.selectedVertices[0] else DISABLED
            )
        widgetState(  # Can only sort directional graphs with 1+ edges
            self.sortButton,
            (
                NORMAL
                if enable and self.nEdges() > 0 and not self.bidirectionalEdges.get()
                else DISABLED
            ),
        )
    ...
    def makeButtons(self, *args, **kwargs):
        ...
        self.depthFirstTraverseButton = self.addOperation(
            ...
            lambda: self.clickTraverse("depth"),
            helpText="Traverse graph in depth-first order",
            state=DISABLED,
        )
        self.breadthFirstTraverseButton = self.addOperation(
            "Breadth-first Traverse",
            lambda: self.clickTraverse("breadth"),
            helpText="Traverse graph in breadth-first order",
            state=DISABLED,
        )
        self.MSTButton = self.addOperation(
            "Minimum Spanning Tree",
            self.clickMinimumSpanningTree,
            helpText="Compute a minimum spanning tree",
            state=DISABLED,
        )
        self.sortButton = self.addOperation(
            "Topological Sort",
            self.clickTopologicalSort,
    ...
    def clickTraverse(self, kind):
        if self.selectedVertices[0]:
            self.traverseExample(
                kind, self.selectedVertices[0][0], start=self.startMode()
            )
        else:
    ...
    def clickMinimumSpanningTree(self):
        if self.selectedVertices[0]:
            self.minimumSpanningTree(
                self.selectedVertices[0][0], start=self.startMode()
            )
        else:
    ...
    def clickTopologicalSort(self):
        if self.bidirectionalEdges.get():
            ...
            self.setMessage(
                "Sorted vertices: {}".format(" ".join(result))
                if result
                else "Cycle in graph.  Sorting impossble."
            )
