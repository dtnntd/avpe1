...
class OutputBox(object):
    ...
    def __init__(  # Constructor
        ...
        see=(),
    ):  # Scroll to see box plus items to see
        ...
        if outputFont is None:
            outputFont = app.VALUE_FONT
        if outputColor is None:
            outputColor = app.VALUE_COLOR
        self.outputFont, self.outputColor = outputFont, outputColor
        if lineHeight is None:
            lineHeight = textHeight(outputFont, " ")
        self.lineHeight = lineHeight
        if bbox is None:
            canvasBBox = app.visibleCanvas()
            width = nCharacters * textWidth(outputFont, "W") + outputOffset[0] * 2
            height = self.lineHeight * nLines + outputOffset[1] * 2
            bbox = (
                max(borderWidth, canvasBBox[0] + canvasBBox[2] - width) // 2,
                canvasBBox[3] - height - outputOffset[1],
                max(borderWidth, canvasBBox[0] + canvasBBox[2] - width) // 2 + width,
    ...
    def drawOutputBoxAndText(self, see=()):
        ...
        self.outputText = self.app.canvas.create_text(
            *(V(self.bbox[:2]) + V(self.outputOffset)),
        ...
        for event, handler in self.eventHandlers:
            if (
                callable(handler)
                and hasattr(OutputBox, handler.__name__)
    ...
    def labelCoords(self):
        center = self.center()
        x = (
            self.bbox[0] - self.labelOffset
            if W in self.labelPosition
            else (
                self.bbox[2] + self.labelOffset
                if E in self.labelPosition
                else center[0]
            )
        )
        y = (
            self.bbox[1] - self.labelOffset
            if N in self.labelPosition
            else (
                self.bbox[3] + self.labelOffset
                if S in self.labelPosition
                else center[1]
            )
        )
        return x, y
    ...
    def appendText(
        ...
        text = (
            textOrItems
            if isinstance(textOrItems, str)
            else self.app.canvas_itemConfig(textOrItems[0], "text")
        )
        current = self.text()
        ...
        while lines and textWidth(self.outputFont, lines[-1]) > maxWidth:
            separators = [
                (mg.group(0), mg.start(), mg.end())
                for mg in self.separators.finditer(lines[-1])
            ]
            if len(separators) == 0:
                separators = [("", 0, len(lines[-1]) // 2)]
            j = len(separators) - 1
            while (
                j > 0
                and textWidth(
                    self.outputFont, lines[-1][: separators[j][1]] + separators[j][0]
                )
                > maxWidth
            ):
                j -= 1
            lines[-1:] = [
                lines[-1][: separators[j][1]] + separators[j][0],
        ...
        if color:
            self.background = (
                ...
                if isinstance(color, str)
                    ...
                    self.app.canvas_itemConfig(nontext1, "fill")
                    if nontext1
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-s", "--sleep-time", default=0.01, type=float, help="Sleep time for animation."
    )
    ...
    outbox1 = OutputBox(
        app,
        (400 - width // 2 - 5, 190, 400 + width // 2 + 5, 190 + height),
        label="Output Item",
    )
    ...
    for j in range(2, 6):
        x = j * (width + gap) - 240
        shape = (app.canvas.create_rectangle if j % 2 == 0 else app.canvas.create_oval)(
            x,
            y0 + dy * j,
            x + width,
            y0 + dy * j + height,
            fill=drawnValue.palette[j],
            outline="",
            width=0,
        )
    ...
    for dataItem in dataItems:
        print(
            "Setting output item to",
            app.canvas_itemConfig(dataItem[1], "text"),
            "with anchor",
            app.canvas_itemConfig(dataItem[1], "anchor"),
            "and sleepTime",
            st,
        )
    ...
    items = [
        app.canvas.create_text(
            ...
            random.randrange(region[1], region[3]),
            text=txt,
            fill="blue",
    ...
    print("Created {} pairs of duplicate word items".format(len(duplicate_items)))
    ...
    for item in items:
        outbox2.appendText(
            item, sleepTime=args.sleep_time, see=outbox2.items(), expand=True
        )
        app.wait(0.01)
    ...
    for pair in duplicate_items:
        outbox2.appendText(
            pair, sleepTime=args.sleep_time, see=outbox2.items(), expand=True
        )
