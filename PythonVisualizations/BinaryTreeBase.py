...
class BinaryTreeBase(VisualizationApp):
    ...
    def setTreeSize(
        ...
        self.STACK_0 = (
            X1 + (self.stackWidth - self.STACK_CELL_SIZE[0]) // 2,
    ...
    def isRoot(self, node):
        return (self.nodes[0] is node) if isinstance(node, Node) else (node == 0)
    ...
    def getLevel(self, node):
        if self.isRoot(node):
            ...
            return self.getLevel(self.getParent(node)) + 1
    ...
    def getHeight(self, node):
        ...
        if n is None:
            return 0
        return max(self.getHeight(child) for child in self.getChildren(node)) + 1
    ...
    def getNodeTree(self, node, erase=False):
        ...
        return [
            node,
            self.getNodeTree(self.getLeftChildIndex(nodeIndex), erase=erase),
            self.getNodeTree(self.getRightChildIndex(nodeIndex), erase=erase),
        ]
    ...
    def storeNodeTree(
        ...
        empty = (
            nodeTree is None or len(nodeTree) != 3 or not isinstance(nodeTree[0], Node)
        )  # Is this an empty node?
        cutoff = (
            (nodeTree[0],)  # Cutoff non-empty nodes
            if not empty and len(self.nodes) <= index  # Beyond limit
            else ()
    ...
    def createTreeObject(
        ...
        x0, y0, x1, y1 = self.treeObjectCoords(offsetAngle, offset, fields, fieldFont)
        rect = self.canvas.create_rectangle(
            x0, y0, x1, y1, fill=color, tags=(label, "treeObject")
        )
        fieldLabels = [
            self.canvas.create_text(
                x0 + sum(fieldWidths[:i]) + fieldWidths[i] // 2,
                y0 + ffHeight,
                text=field,
                font=fieldFont,
                fill="black",
        ...
        self.treeObject = (rect, arrow, rootLabel, oval, *fieldLabels, treeLabel)
    ...
    def treeObjectFonts(self, font=None):
        fieldFont = (
            self.VARIABLE_FONT[0] if font is None else font[0],
    ...
    def treeObjectFieldWidths(self, fields=[], font=None):
        if font is None:
            fieldFont, _ = self.treeObjectFonts()
        return [textWidth(fieldFont, " {} ".format(field)) for field in fields]

    def treeObjectCoords(self, offsetAngle=None, offset=None, fields=[], font=None):
        fieldFont, _ = self.treeObjectFonts(font)
        ffHeight = textHeight(fieldFont)
        rootWidth = textWidth(fieldFont, " root ")
        fieldsWidth = sum(
            textWidth(fieldFont, " {} ".format(field)) for field in fields
        )
        if offset is None:
            offset = 40
    ...
    def treeObjectArrowCoords(self, **kwargs):
        ...
        dotCenter = V(treeObjectCoords[:2]) + V(
            self.treeDotCenter(
                fields=kwargs.get("fields", []), font=kwaargs.get("font")
            )
        )
        tip = (
            (
                V(self.ROOT_X0, self.ROOT_Y0)
                + V(self.CIRCLE_SIZE, 0).rotate(kwargs.get("offsetAngle", 180))
            )
            if self.getRoot()
            else dotCenter
        )
    ...
    def updateTreeObjectRootPointer(self, arrow=None, root=None):
        ...
        self.canvas.coords(
            ...
            *(
                arrowCoords[:2]
                if root is None
                else V(root.center) - V(self.CIRCLE_SIZE, 0)
            )
        )
    ...
    def createArrow(
        ...
        label = self.canvas.create_text(
            *labelCoords, text=label, anchor=anchor, font=font, fill=color, tags=tags
        )
        return (arrow, label)
        ...
        **kwargs
        ...
        center = (
            ...
            if isinstance(nodeOrCoords, Node)
                ...
                self.nodeCenter(nodeOrCoords)
                if isinstance(nodeOrCoords, int)
                else nodeOrCoords
            )
        )
        ...
        base = V(
            self.CIRCLE_SIZE
            + (0 if level == 0 else self.ARROW_HEIGHT)
            + level * abs(font[1]),
            0,
        ).rotate(orientation)
    ...
    def moveArrow(
        ...
        **kwargs: "Keyword args for indexCoords"
        ...
        self.moveItemsTo(
            arrow,
            (newArrow,) if len(arrow) == 1 else (newArrow, newLabel),
            steps=numSteps,
            sleepTime=sleepTime,
        )
    ...
    def nodeCenter(self, node):
        ...
        if node < 0:
            treeObjectBox = (
                (0, 0, 40, 30)
                if getattr(self, "treeObject", None) is None
                else self.canvas.coords(self.treeObject[0])
            )
    ...
    def createNodeHighlight(self, node, highlightWidth=2, color=None, radius=None):
        if color is None:
    ...
    def createHighlightedLine(
        ...
        if line is None:
            return
        if color is None:
            color = drawnValue.palette[0]
        tags = ["highlight", "line"] + (
            []
            if tags is None
            else tags.split() if isinstance(tags, str) else list(tags)
        )
        highlightLine = self.canvas.copyItem(line)
        self.canvas.tag_lower(highlightLine, self.getRoot().drawnValue.items[1])

        self.canvas.itemconfig(highlightLine, fill=color, width=width, tags=tags)

        if callEnviron:
            callEnviron.add(highlightLine)
    ...
    def createNodeShape(
        ...
        circle = self.canvas.create_oval(*coords[1], tag=tag, fill=color, outline="")
        circle_text = self.canvas.create_text(*coords[2], text=key, font=font, tag=tag)
        self.canvas.tag_lower(line)
        for item in (circle, circle_text):
            self.canvas.tag_bind(item, "<Button>", lambda e: self.setArgument(key))
    ...
    def restoreNodePositions(self, nodes, sleepTime=0.05, includeLines=True):
        ...
        for node in nodes:
            ...
            for item, coords in zip(
                ...
                self.nodeItemCoords(node.center, parent=self.getParentIndex(nodeIndex)),
            ):
    ...
    def outputBoxCoords(self, font=None, padding=6, N=None, canvasDims=None):
        ...
        return (
            left,
            canvasDims[1] - height - padding,
            left + width,
            canvasDims[1] - padding,
        )
    ...
    def createLine(self, node):
        line = self.canvas.create_line(
            *self.lineCoordinates(node), tags=("line", node.tag + "_line"), arrow=FIRST
        )
        self.canvas.tag_lower(line)
    ...
    def createNode(
        self, key, parent=None, direction=None, color=None, addToArray=True, center=None
    ):
        ...
        drawnValueObj = drawnValue(
            key, *self.createNodeShape(*center, key, tag, parent=None, color=color)
        )
    ...
    def copyNode(self, node):
        ...
        drawnValueObj = drawnValue(
            key, *self.createNodeShape(*node.center, node.getKey(), tag)
        )
    ...
    def setLeftChild(self, node, child, updateLink=False):
        ...
        if index != -1 and index < len(self.nodes):
            ...
            if updateLink and child and child.getLine():
                self.canvas.coords(child.getLine(), *self.lineCoordinates(child, node))
            return index
        else:
            return -1
        ...
        if child:
            if nodeIndex < 0:  # When the parent is tree object
                ...
                self.moveItemsLinearly(
                    (childLine, highlightLine),
                    (newLineCoords, newLineCoords),
                    sleepTime=wait / 10,
                )
    ...
    def moveSubtree(
        ...
        return self.storeNodeTree(self.getNodeTree(fromIndex, erase=True), toIndex)
    ...
    def clearDescendants(self, node, subSize=True):
        ...
        if not node or nodeIndex == -1 or nodeIndex >= self.maxElems:
            return
    ...
    def display(self, fields=[], treeLabel="BinarySearchTree"):
        ...
        newNodes = [
            (
                self.createNode(
                    node.getKey(),
                    None if i == 0 else self.nodes[(i - 1) // 2],
        ...
        depthBoundary = self.nodeCenter(len(self.nodes) + 1) + self.nodeCenter(
            len(self.nodes) * 2
        )
        self.canvas.create_line(
            *depthBoundary,
            fill=self.DEPTH_BOUNDARY_COLOR,
            dash=self.DEPTH_BOUNDARY_DASH,
            tags="boundary"
        )
    ...
    def fill(self, values, animation=False):
        ...
        nums = (
            random.sample(range(self.valMax), values)
...
"""
    ...
    def _find(self, goal, animation=True, code=_findCode):
        wait = 0.1
        callEnviron = self.createCallEnvironment(
            code="" if not animation else code.format(**locals()), sleepTime=wait / 10
        )
...
"""
    ...
    def search(self, goal, code=searchCode, start=True):
        wait = 0.1
        callEnviron = self.createCallEnvironment(
            code=code.format(**locals()), startAnimations=start, sleepTime=wait / 10
        )
    ...
    def insert(self, key, animation=True, code=insertCode, start=True):
        ...
        if node < self.maxElems:
            newNode = self.createNode(
                key,
                parent=None if parent < 0 else self.nodes[parent],
                direction=(
                    Child.LEFT
                    if parent < 0 or key < self.nodes[parent].getKey()
                    else Child.RIGHT
                ),
                addToArray=False,
        ...
        if self.getNode(node):
            ...
            animation
            and self.highlightCode("parent is self", callEnviron, wait=wait)
            or parent == -1
        ):
            if animation:
                self.highlightCode(
                    "self.__root = self.__Node(key, data)", callEnviron, wait=wait
                )
                self.moveItemsTo(
                    newNode.drawnValue.items,
                    self.nodeItemCoords(0),
                    sleepTime=wait / 10,
                )
            ...
            if animation:
                self.setMessage(
                    "Error! Cannot insert at level {} or below".format(self.MAX_LEVEL)
                )
            ...
            if animation:
                ...
                self.moveItemsLinearly(
                    newNode.drawnValue.items,
                    self.nodeItemCoords(node, parent),
                    sleepTime=wait / 10,
                )
                if dir == "left":
...
"""
    ...
    def traverseExample(
        ...
        callEnviron = self.createCallEnvironment(
            code=code.format(**locals()), sleepTime=wait / 10, startAnimations=start
        )
        ...
        outputBox = self.createOutputBox(coords=outBoxCoords, outputOffset=(5, 10))
        callEnviron |= set(outputBox.items())

        iteratorCall = "key, data in tree.traverse({traverseType!r})".format(**locals())
        self.iteratorStack = []
        ...
        for key, items in self.traverse(traverseType):
            ...
            if dataIndex is None:
                dataIndex = self.createArrow(nodeindex, "key, data", orientation=-135)
                callEnviron |= set(dataIndex)
                localVars += dataIndex
            else:
                self.moveItemsTo(
                    dataIndex,
                    self.indexCoords(nodeindex, 1, orientation=-135),
                    sleepTime=wait / 10,
                )

            self.highlightCode("print(key)", callEnviron, wait=wait)
            keyItem = self.canvas.copyItem(self.getNode(nodeindex).drawnValue.items[2])
...
"""
    ...
    def traverse(self, traverseType="in", code=traverseCode, wait=0.1):
        ...
        if self.highlightCode(
            "traverseType not in ['pre', 'in', 'post']",
            callEnviron,
            wait=wait,
            returnValue=traverseType not in ["pre", "in", "post"],
        ):
            self.highlightCode(
        ...
        self.traverseStack = Table(
            self,
            self.STACK_0,
            cellWidth=self.STACK_CELL_SIZE[0],
            cellBorderWidth=1,
            cellHeight=self.STACK_CELL_SIZE[1],
            vertical=True,
            direction=-1,
            label="stack",
            labelFont=self.VARIABLE_FONT,
            labelColor=self.VARIABLE_COLOR,
        )
        ...
        root = self.getRoot()
        self.stackPush(
            root, callEnviron, wait=wait, center=self.nodeCenter(0 if root else -1)
        )
        ...
        while len(self.traverseStack) > 0:
            ...
            if itemArrow is None:
                ...
                self.moveItemsLinearly(
                    itemArrow, self.traverseItemArrowCoords(None), sleepTime=0, steps=1
                )

            item = self.stackPop(callEnviron, wait=wait)

            if self.highlightCode(
                "isinstance(item, self.__Node)",
                callEnviron,
                wait=wait,
                ...
                self.moveItemsLinearly(
                    itemArrow,
                    self.traverseItemArrowCoords(item.val),
                    sleepTime=wait / 10,
                )
                if self.highlightCode(
                    "traverseType == 'post'",
                ...
                self.highlightCode("stack.push(item.rightChild)", callEnviron)
                self.stackPush(
                    self.getRightChild(item.val),
                    callEnviron,
                    wait=wait,
                ...
                self.highlightCode("stack.push(item.leftChild)", callEnviron),
                self.stackPush(
                    self.getLeftChild(item.val),
                    callEnviron,
                    wait=wait,
                ...
                ("item", 11), callEnviron, wait=wait, returnValue=item.val is not None
            ):
                self.highlightCode("yield item", callEnviron, wait=wait)
                itemCoords = self.yieldCallEnvironment(callEnviron, sleepTime=wait / 10)
                yield item.val, item.items
                self.resumeCallEnvironment(callEnviron, itemCoords, sleepTime=wait / 10)
        ...
        if isinstance(thing, Node):
            ...
            stackRect = self.canvas.create_rectangle(
                ...
                *(V(V(center) + V(cellSize)) - V(halfCell)),
                fill=self.canvas_itemConfig(nodeItems[1], "fill"),
                outline="",
                width=0
            )
            ...
            toMove = thing.items
            moveTo = (
                (cellCoords[0] + halfCell[0], *cellCoords[1:]),
                (
            ...
            noneDot = self.canvas.create_oval(
                *(V(center) - dotRadius),
                *(V(center) + dotRadius),
                fill=self.NONE_DOT_COLOR,
                outline="",
                width=0
            )
            callEnviron.add(noneDot)
            toMove = (noneDot,)
            moveTo = ((V(cellCenter) - dotRadius) + (V(cellCenter) + dotRadius),)

        self.moveItemsLinearly(
            toMove, moveTo, sleepTime=wait / 10, startFont=startFont, endFont=endFont
        )
    ...
    def stackPop(
        ...
        self.moveItemsBy(
            top.items, (0, labelCoords[1] - topCenter[1]), sleepTime=wait / 10
        )
    ...
    def validArgument(self):
        ...
        if entered_text and entered_text.isdigit():
            ...
            if val <= self.valMax:
                ...
                self.setMessage(
                    "Input value must be an integer from 0 to {}.".format(self.valMax)
                )
    ...
    def clickSearch(self):
        ...
        if val:
            self.setMessage(
                "Key {} not found".format(val)
                if self.search(val, start=self.startMode()) is None
                else "Found key {}".format(val)
            )
            self.clearArgument()
    ...
    def __pTree(self, node, indent, indentBy, **kwargs):
        if node:
            ...
            self.__pTree(self.getLeftChild(node), indent + indentBy, indentBy, **kwargs)
        ...
        for i, node in enumerate(self.nodes):
            if node is not None and node not in allNodes:
                ...
                if 0 <= i and self.nodes[parentIndex] is not None:
                    print(
                        " but parent at {} contains {}".format(
                            parentIndex, self.nodes[parentIndex]
                        ),
                        **kwargs
                    )
