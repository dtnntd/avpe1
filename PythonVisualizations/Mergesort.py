...
"""
    ...
    def mergesortInit(self, code=initMergesortCode, start=True):
        ...
        for i in range(self.size):
            self.highlightCode("self.__work.insert(None)", callEnviron, wait=wait / 50)
        ...
        if len(self.arrayCells) == self.size:  # Create an invisible array cell
            self.arrayCells.append(
                self.createArrayCell(  # at the right end
                    len(self.arrayCells),  # to use for hi index
                    tags=["hidden", "arrayBox"],
                    width=0,
    ...
    def mergesort(self, lo, hi, level=0, code=mergesortCode):
        ...
        for idx, val in (("loIndex", lo), ("hiIndex", hi)):
            ...
            if getattr(self, idx, None) is None:
                setattr(self, idx, self.createCellIndex(arrayCell, idx[:2], level=2))
        ...
        self.moveItemsBy(
            flat(
                *(
                    [self.arrayCells[j]] + [item for item in self.list[j].items]
...
"""
    ...
    def merge(self, lo, mid, hi, code=mergeCode):
        ...
        idxLo = lo
        idxLoIndex = self.createCellIndex(self.arrayCells[idxLo], "idxLo", level=-1)
        callEnviron.add(idxLoIndex)

        self.highlightCode("idxHi = mid", callEnviron, wait=wait)
        idxHi = mid
        idxHiIndex = self.createCellIndex(self.arrayCells[idxHi], "idxHi", level=-2)
        ...
        while idxLo < mid and idxHi < hi:
            self.highlightCode("itemLo = self.__arr.get(idxLo)", callEnviron, wait=wait)
            self.highlightCode("itemHi = self.__arr.get(idxHi)", callEnviron, wait=wait)
            self.highlightCode(
                "self.__key(itemLo) <= self.__key(itemHi)", callEnviron, wait=wait
            )
        ...
        while idxLo < mid:
            self.highlightCode("self.__work.set(n, self.__arr.get(idxLo))", callEnviron)
            copiedVal = self.setArrayValue(
                self.workArray,
                self.workCells,
                n,
    ...
    def moveIndex(self, index, rectID):  # Move an index to be above a rectangle
        self.canvas.coords(
            index, (self.cellIndexCoords(rectID, 1)[0], self.canvas.coords(index)[1])
        )

    def setArrayValue(
        self, toArray, toCells, toIndex, fromArray, fromCells, fromIndex, callEnviron
    ):
    ...
    def makeButtons(self, maxRows=4):
        ...
        mergesortButton = self.addOperation(
            "Mergesort",
            lambda: self.mergesortInit(start=self.startMode()),
            maxRows=maxRows,
